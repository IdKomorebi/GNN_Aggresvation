"""顺序运行DNN_Aggresvation15的局部补边实验，并生成汇总表。"""
...
CONFIGS = [
    PROJECT_ROOT / "configs" / "tuning" / "target_extra_edges_allloss_net16_gross16_losses22.yaml",
]
...
def main() -> None:
    ...
    subprocess.run(
        [
            ...
            str(compare_script),
            "--outputs",
            str(PROJECT_ROOT / "outputs_target_extra"),
        ],
        cwd=str(PROJECT_ROOT.parent),
        check=True,
    )
