"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation37）。

在前序基础上新增 **K 专家 + 全局 per-target 软分配（分辨率代偿）**：
DNN34（路由）/35（对角 value）/36（低秩 value）证明「在注意力下游做 per-target
修正」无系统增益，瓶颈在共享表示。DNN37 换思路——不追求每字段一个专属通道
（=single），而是让 12 个字段共享 **K 个专家**（只专属 value 投影 + 输出头），
每个字段用一个**全局软分配 π_c ∈ Δ^K**（低温 softmax）组合这 K 个专家：

    msg_c   = Σ_k π_c[k] · (Σ_j α(c,j)·v^k_j)      # value 专家按 π 组合
    pred_c  = Σ_k π_c[k] · head^k(h_conf_c)         # 输出头按 π 组合

12 个字段的有效自由度 ≤ K，即「等价于推断 K 个字段」。K=value_experts：
- K=1  → 全共享（≈ multi 下界）
- K=12 + π 趋 one-hot → 每字段独占一专家（≈ single 上界）
- K=3/4/6 → 中间代偿带（牺牲精度换低成本）

低温 expert_route_temperature 促 π_c 稀疏（逼近"每字段选少数专家"）；
expert_diversity_weight 加负载均衡 aux loss，防止所有字段塌缩到同一专家。
q/k/gate/attention 与图结构共享（单头）。
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        value_experts: int = 0,
        expert_route_temperature: float = 0.5,
        ...
        if int(value_experts) < 0:
            raise ValueError(f"value_experts must be >= 0, got {value_experts}")
        ...
        # 专家模式（DNN37）：K 个专家，全局 per-target 软分配 π_c 组合
        self.value_experts = int(value_experts)
        self.use_experts = self.value_experts >= 1
        self.expert_route_temperature = max(float(expert_route_temperature), 1e-3)
        ...
        # 专家模式（DNN37）：每层 K 个独立 value 投影；attention/q/k/gate 用单头共享。
        self.expert_value_layers = nn.ModuleList()  # per layer: ModuleList of K Linear
        ...
        for _ in range(num_layers):
            ...
            # 专家模式：每层 K 个独立 value 投影
            if self.use_experts:
                self.expert_value_layers.append(nn.ModuleList([
                    nn.Linear(hidden_dim, hidden_dim, bias=False)
                    for _ in range(self.value_experts)
                ]))
        ...
        # 专家模式：全局 per-target 软分配 π_c（由 target 身份生成）+ K 个专家输出头
        if self.use_experts:
            self.expert_route_layer = nn.Linear(hidden_dim, self.value_experts, bias=True)
            self.expert_output_heads = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(hidden_dim, hidden_dim // 2),
                    nn.ReLU(),
                    nn.Linear(hidden_dim // 2, 1),
    ...
    def _expert_route(self) -> torch.Tensor:
        """全局 per-target 软分配 π_c ∈ Δ^K（低温 softmax），形状 (C, K)。

        π_c[k] = 字段 c 分配到专家 k 的程度。K=value_experts。
        """
        logits = self.expert_route_layer(self.target_identity)  # (C, K)
        return torch.softmax(logits / self.expert_route_temperature, dim=-1)
    ...
    def _confidential_messages(
        ...
        # ---- 专家模式（DNN37）：共享单头 attention + K 个 value 专家按 π_c 组合 ----
        if self.use_experts:
            q = (
                self.query_hidden_layers[layer_idx][0](h_conf)
                + self.query_identity_layers[layer_idx][0](target_id)
            )
            k = self.key_layers[layer_idx][0](h_src)
            ...
            if self.architecture == "hybrid_bilinear_prior_only":
                ...
                score = dynamic_score + scales[0] * prior_score
                gate = torch.ones_like(score)
            else:
                gate_logits = (
                    self.gate_q_layers[layer_idx][0](q)
                    + self.gate_k_layers[layer_idx][0](k).transpose(1, 2)
                    + self.gate_p_layers[layer_idx][0](prior_score.unsqueeze(-1)).squeeze(-1)
                )
                gate = torch.sigmoid(gate_logits)
                score = gate * dynamic_score + (1.0 - gate) * scales[0] * prior_score
            ...
            route = self._expert_route()  # (C, K)
            msg = h_conf.new_zeros(B, self.n_confidential, self.hidden_dim)
            for k_idx in range(self.value_experts):
                v_k = self.expert_value_layers[layer_idx][k_idx](h_src)  # (B, S, hidden)
                m_k = torch.bmm(attention, v_k)                          # (B, C, hidden)
                msg = msg + route[:, k_idx].view(1, -1, 1) * m_k
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        # 专家模式：K 个专家输出头按 π_c 组合 + 多样性/负载均衡 aux loss
        if self.use_experts:
            route = self._expert_route()  # (C, K)，带梯度
            outs = torch.stack(
                [head(h_conf).squeeze(-1) for head in self.expert_output_heads],
                dim=-1,
            )  # (B, C, K)
            pred = (outs * route.unsqueeze(0)).sum(dim=-1)  # (B, C)
            if self.expert_diversity_weight > 0:
                # 负载均衡：鼓励 K 个专家被字段均衡使用（防止塌缩到单专家）。
                # balance ∈ [0, K-1]，=0 当完全均衡。
                mean_route = route.mean(dim=0)  # (K,)
                balance = (mean_route * mean_route).sum() * self.value_experts - 1.0
                self.aux_loss = self.expert_diversity_weight * balance
            else:
                self.aux_loss = None
            return pred
