#!/usr/bin/env python3
"""DNN37 调度器：4 GPU 并行跑 configs/ 下的全部实验。

DNN37 共 5 组：experts_k{1,3,4,6,12}——K 个专家（分辨率旋钮），
用于画 K–精度代偿曲线。
...
D37 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation37")
...
configs = [D37 / "configs" / f"{n}.yaml" for n in order]
...
print(f"DNN37 调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D37 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
