"""
DNN_Aggresvation34 主流水线脚本。

在 DNN32 最优配置基础上引入真正的双重注意力机制：
- 每个 head 拥有独立的完整维度 q/k/v/gate（不切维度）
- target-conditioned head gate 加权合并各 head 消息
- 固定 bipartite=True, shuffle split, nostaged, allloss
- 输出目录结构：outputs/<model_variant>/<timestamp>/

与 DNN33 区别：DNN33 切了维度（head_dim=att_dim/n_heads），DNN34 不切。
"""
...
def _load_or_compute_metric_tensor(
    ...
    单目标模式下，data_info 的 all_columns 是全图的子集。如果全图缓存
    存在且 metrics 匹配，则从全图缓存中按列索引提取子集，避免重新计算。
    """
    ...
    if cache_tensor and cache_tensor.exists() and cache_meta and cache_meta.exists():
        with open(cache_meta, "r", encoding="utf-8") as f:
            meta = json.load(f)
        if meta.get("metrics") == metrics:
            cache_columns = meta.get("columns")
            current_columns = data_info["all_columns"]
            if cache_columns == current_columns:
            ...
            # 单目标模式：从全图缓存按列子集提取
            if set(current_columns).issubset(set(cache_columns)):
                print(f"  从全图缓存提取子集: {cache_tensor}")
                full_tensor = np.load(cache_tensor)
                idx_map = [cache_columns.index(c) for c in current_columns]
                metric_tensor = full_tensor[np.ix_(idx_map, idx_map)]
                sub_meta = {"metrics": metrics, "columns": current_columns}
                np.save(rel_dir / "metric_tensor.npy", metric_tensor)
                _save_json(rel_dir / "metrics.json", sub_meta)
                return metric_tensor
            print("  相关性缓存字段不匹配，将重新计算。")
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN25混合聚合的alpha、prior、gate诊断。"""
    ...
    print(f"  DNN25混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation25: staged all-loss training pipeline"
    )
    parser.add_argument(
        "--config",
    ...
    # 创建运行目录：outputs/<model_variant>/<timestamp>/
    single_target = cfg.get("fields", {}).get("single_target", None)
    n_heads = int(cfg.get("model", {}).get("n_heads", 1))
    head_aggregation = str(cfg.get("model", {}).get("head_aggregation", "static_gate"))
    value_film = str(cfg.get("model", {}).get("value_film", "none"))
    value_lora_rank = int(cfg.get("model", {}).get("value_lora_rank", 0))
    value_experts = int(cfg.get("model", {}).get("value_experts", 0))
    if single_target:
        model_variant = f"single_{single_target}"
    else:
        # DNN35：直接用 experiment.name 作为输出目录名（完全可控）
        model_variant = experiment_name
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = output_root / model_variant / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation34: dual attention (full-dim independent heads + target gate)")
    print("=" * 60)
    print(f"  实验名称: {experiment_name}")
    print(f"  模型变体: {model_variant}")
    print(f"  n_heads: {n_heads}")
    print(f"  head_aggregation: {head_aggregation}")
    print(f"  value_film: {value_film}")
    print(f"  value_lora_rank: {value_lora_rank}")
    print(f"  value_experts: {value_experts}")
    ...
    model_cfg_early = cfg.get("model", {})
    use_bipartite = bool(model_cfg_early.get("bipartite", False))
    ...
    print(f"  bipartite: {use_bipartite}")
    ...
    model = InferenceDrivenGNN(
        ...
        input_encoder=str(model_cfg.get("input_encoder", "linear")),
        input_encoder_dropout=float(model_cfg.get("input_encoder_dropout", 0.0)),
        bipartite=use_bipartite,
        n_heads=int(model_cfg.get("n_heads", 1)),
        head_gate_temperature=float(model_cfg.get("head_gate_temperature", 1.0)),
        head_gate_bias_init=float(model_cfg.get("head_gate_bias_init", 0.0)),
        head_aggregation=str(model_cfg.get("head_aggregation", "static_gate")),
        value_film=str(model_cfg.get("value_film", "none")),
        value_lora_rank=int(model_cfg.get("value_lora_rank", 0)),
        value_experts=int(model_cfg.get("value_experts", 0)),
        expert_route_temperature=float(model_cfg.get("expert_route_temperature", 0.5)),
        expert_diversity_weight=float(model_cfg.get("expert_diversity_weight", 0.0)),
    ).to(device)
    ...
    print(f"  input_encoder: {str(model_cfg.get('input_encoder', 'linear'))}")
    print(f"  input_encoder_dropout: {float(model_cfg.get('input_encoder_dropout', 0.0))}")
    print(f"  bipartite: {use_bipartite}")
