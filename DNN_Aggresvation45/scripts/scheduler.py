#!/usr/bin/env python3
"""DNN45 消融验证调度器：4 GPU 并行跑 configs/ 下的全部验证配置。
...
D45 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation45")
PY = "/data1/duhaocun/miniconda3/envs/Pytorch310_codex/bin/python"
GPUS = [0, 1, 2, 3]
LOG_DIR = D45 / "logs"
LOG_DIR.mkdir(exist_ok=True)

configs = sorted((D45 / "configs").glob("*.yaml"))

queue = deque(configs)
gpu_pool = list(GPUS)
running = {}  # gpu -> (proc, config_name)

print(f"DNN45 消融验证调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D45 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
