...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        self.edge_value_layers = nn.ModuleList()
        ...
        for _ in range(num_layers):
            ...
            edge_value_layer = nn.Sequential(
                nn.Linear(self.n_metrics + 2, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, hidden_dim * 2),
            )
            nn.init.zeros_(edge_value_layer[-1].weight)
            nn.init.zeros_(edge_value_layer[-1].bias)
            self.edge_value_layers.append(edge_value_layer)
    ...
    def get_alpha_general(self) -> torch.Tensor:
        if self.alpha_mode == "uniform_fixed":
            return torch.full_like(self.beta_general, 1.0 / self.n_metrics)
        return torch.softmax(self.beta_general / self.edge_alpha_temperature, dim=0)

    def get_alpha_confidential(self) -> torch.Tensor:
        if self.alpha_mode == "uniform_fixed":
            return torch.full_like(self.beta_confidential, 1.0 / self.n_metrics)
    ...
    def _confidential_edge_features(self, prior: torch.Tensor) -> torch.Tensor:
        """Static target-source edge features for value conditioning, shape (C,S,K+2)."""
        rel = self.metric_tensor.index_select(0, self.conf_indices)
        if self.bipartite:
            rel = rel.index_select(1, self.general_indices)
            prior = prior.index_select(1, self.general_indices)
        prior_log = torch.log(prior.clamp(min=self.prior_log_eps))
        return torch.cat([rel, prior.unsqueeze(-1), prior_log.unsqueeze(-1)], dim=-1)
    ...
    def _confidential_messages(
        ...
        elif self.architecture == "hybrid_bilinear_dynamic_only":
            score = dynamic_score
        ...
        if self.value_mode == "edge_film":
            edge_features = self._confidential_edge_features(full_prior)
            film = self.edge_value_layers[layer_idx](edge_features)
            delta_gamma, delta_bias = film.chunk(2, dim=-1)
            scale = self.edge_value_scale
            v_edge = (
                v.unsqueeze(1)
                * (1.0 + scale * torch.tanh(delta_gamma).unsqueeze(0))
                + scale * torch.tanh(delta_bias).unsqueeze(0)
            )
            return torch.sum(attention.unsqueeze(-1) * v_edge, dim=2)
