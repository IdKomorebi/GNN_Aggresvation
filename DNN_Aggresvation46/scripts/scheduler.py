#!/usr/bin/env python3
"""DNN46 edge-conditioned value 调度器：4 GPU 并行跑 configs/ 下的全部验证配置。
...
D46 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation46")
PY = "/data1/duhaocun/miniconda3/envs/Pytorch310_codex/bin/python"
GPUS = [0, 1, 2, 3]
LOG_DIR = D46 / "logs"
LOG_DIR.mkdir(exist_ok=True)

configs = sorted((D46 / "configs").glob("*.yaml"))

queue = deque(configs)
gpu_pool = list(GPUS)
running = {}  # gpu -> (proc, config_name)

print(f"DNN46 edge-conditioned value 调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D46 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
