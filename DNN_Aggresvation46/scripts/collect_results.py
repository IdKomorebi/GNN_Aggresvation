#!/usr/bin/env python3
"""Collect DNN46 edge-conditioned value results into comparison CSV files."""
...
def main() -> None:
    ...
    for exp_dir in sorted(TUNING_DIR.glob("ev*")):
    ...
    summary_df.to_csv(OUT_DIR / "edge_value_summary_multi.csv", index=False)
    target_df.to_csv(OUT_DIR / "edge_value_per_target_multi.csv", index=False)

    print(summary_df.to_string(index=False))
    print(f"\nSaved: {OUT_DIR / 'edge_value_summary_multi.csv'}")
    print(f"Saved: {OUT_DIR / 'edge_value_per_target_multi.csv'}")
