...
def main() -> None:
    ...
    model = InferenceDrivenGNN(
        ...
        bipartite=use_bipartite,
        alpha_mode=str(model_cfg.get("alpha_mode", "learned")),
        value_mode=str(model_cfg.get("value_mode", "source")),
        edge_value_scale=float(model_cfg.get("edge_value_scale", 0.25)),
    ).to(device)
    ...
    print(f"  bipartite: {use_bipartite}")
    print(f"  alpha_mode: {str(model_cfg.get('alpha_mode', 'learned'))}")
    print(f"  value_mode: {str(model_cfg.get('value_mode', 'source'))}")
    print(f"  edge_value_scale: {float(model_cfg.get('edge_value_scale', 0.25))}")
