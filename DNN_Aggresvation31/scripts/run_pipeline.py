"""
DNN_Aggresvation31 主流水线脚本。

在 DNN30 shuffle split 基础上，做 2×2 因子实验：
- {staged, nostaged} × {maskloss, allloss}
- 固定 bipartite=True, split_mode=shuffle
- 输出目录结构：outputs/<staging_mode>/<loss_mode>/<timestamp>/

目的：
1. staged training 在 shuffle split 下是否仍有必要
2. allloss（加入 net/gross）是否损害其它 10 个字段的预测
3. 多目标联合推断 vs 单独推断的质量差异
"""
...
def main() -> None:
    ...
    # 创建运行目录：outputs/<staging_mode>/<loss_mode>/<timestamp>/
    staged_cfg = cfg.get("training", {}).get("staged_training", {})
    staging_mode = "staged" if bool(staged_cfg.get("enabled", False)) else "nostaged"
    loss_excluded = cfg.get("training", {}).get("loss_exclude_confidential", [])
    loss_mode = "maskloss" if loss_excluded else "allloss"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = output_root / staging_mode / loss_mode / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation31: staged vs nostaged x maskloss vs allloss")
    print("=" * 60)
    print(f"  实验名称: {experiment_name}")
    print(f"  staging: {staging_mode}")
    print(f"  loss口径: {loss_mode}")
