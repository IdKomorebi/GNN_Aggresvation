...
目的：
- 每个Confidential字段单独训练一个MLP，只使用General字段作为输入
- 判断低R²字段是“数据本身不可推断”，还是“多目标共享图结构拖累”
- 与DNN_Aggresvation4已有GNN/GAT结果做横向对比
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run single-target MLP probes")
    parser.add_argument(
        "--config",
        default=str(PROJECT_ROOT / "configs" / "experiments" / "baseline_gcn.yaml"),
    )
    ...
    run_id = datetime.now().strftime("target_probe_%Y%m%d_%H%M%S")
    run_dir = output_root / run_id
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 72)
    print("  DNN_Aggresvation4 Route A: 单目标MLP上限诊断")
    print("=" * 72)
