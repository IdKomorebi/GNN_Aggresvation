"""
DNN_Aggresvation33 主流水线脚本。

在 DNN32 最优配置基础上引入双重注意力机制（Dual Attention）：
- 多头 confidential attention + target-conditioned head gate
- 固定 bipartite=True, shuffle split, nostaged, allloss
- 输出目录结构：outputs/<model_variant>/<timestamp>/
  model_variant ∈ {baseline_1head, dual_4head, dual_8head}

目的：验证双重注意力能否缩小 multi 模式与 single 模式的 R² 差距。
"""
...
def main() -> None:
    ...
    if single_target:
        ...
        model_variant = f"dual_{n_heads}head"
    else:
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation33: dual attention (multi-head + target-conditioned head gate)")
    print("=" * 60)
