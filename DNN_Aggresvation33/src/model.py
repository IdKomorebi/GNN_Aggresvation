"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation33）。

在 DNN32 bipartite 主干基础上引入双重注意力机制（Dual Attention）：
- 多头 confidential attention：将 attention_dim 切成 n_heads 份，每份
  独立计算 q/k/v 和 attention score，捕获不同类型的 source-target 关系。
- target-conditioned head gate：每个 confidential target 学习自己的
  head 组合权重 head_weight_c = softmax(W_head · target_identity_c / τ)，
  使不同 target 可以偏向不同的 attention head，近似"软性专属通道"。
- 当 n_heads=1 时退化为 DNN32 单头行为。
- bipartite 开关沿用 DNN29：切断 conf-conf 边。
"""
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        _attn_dim = int(attention_dim or hidden_dim)
        if n_heads < 1 or _attn_dim % n_heads != 0:
            raise ValueError(
                f"n_heads={n_heads} must divide attention_dim={_attn_dim}"
            )
        ...
        self.query_hidden_layers = nn.ModuleList()  # per-layer, ModuleList of n_heads
        ...
        self.head_gate_layers = nn.ModuleList()  # target-conditioned head gate
        ...
        for _ in range(num_layers):
            ...
            self.gate_q_layers.append(
                nn.ModuleList([
                    nn.Linear(self.attention_dim, 1, bias=False)
                    for _ in range(self.n_heads)
                ])
            )
            self.gate_k_layers.append(
                nn.ModuleList([
                    nn.Linear(self.attention_dim, 1, bias=False)
                    for _ in range(self.n_heads)
                ])
            )
            self.gate_p_layers.append(
                nn.ModuleList([
                    nn.Linear(1, 1, bias=True)
                    for _ in range(self.n_heads)
                ])
            )
            for hg_p in self.gate_p_layers[-1]:
                nn.init.constant_(hg_p.bias, float(gate_bias_init))
            self.prior_scales.append(
                nn.Parameter(torch.full((self.n_heads,), float(prior_scale_init)))
            )

            # target-conditioned head gate
    ...
    def _confidential_messages(
        ...
        if self.architecture == "hybrid_bilinear_prior_only":
            gate = torch.zeros(B, self.n_confidential, prior.shape[1],
                               device=h.device, dtype=h.dtype)
            use_gate = False
        elif self.architecture == "hybrid_bilinear_no_gate":
            gate = torch.ones(B, self.n_confidential, prior.shape[1],
                              device=h.device, dtype=h.dtype)
            use_gate = False
        else:
            gate_logits = (
        ...
        # ---- multi-head dual attention ----
        # q: (B, C, D), k: (B, S, D), v: (B, S, H)
        # 切分成 n_heads 份，每份 head_dim 维
        H = self.n_heads
        Dh = self.head_dim
        qh = q.view(B, self.n_confidential, H, Dh).transpose(1, 2)    # (B, H, C, Dh)
        kh = k.view(B, -1, H, Dh).transpose(1, 2)                     # (B, H, S, Dh)
        vh = v.view(B, -1, H, Dh).transpose(1, 2)                     # (B, H, S, Dh)

        # per-head dynamic score: (B, H, C, S)
        dyn = torch.matmul(qh, kh.transpose(-2, -1)) / math.sqrt(Dh)

        # per-head gate and score
        # gate: (B, C, S) -> (B, 1, C, S) broadcast over heads
        gate_exp = gate.unsqueeze(1)  # (B, 1, C, S)
        prior_exp = prior_score.unsqueeze(1)  # (B, 1, C, S)
        valid_exp = valid.unsqueeze(0).unsqueeze(0)  # (1, 1, C, S)

        if use_gate:
            score_h = gate_exp * dyn + (1.0 - gate_exp) * scale * prior_exp
        else:
            score_h = dyn + scale * prior_exp

        score_h = score_h / self.attention_temperature
        score_h = score_h.masked_fill(~valid_exp, -1e9)
        attn_h = torch.softmax(score_h, dim=-1)
        attn_h = attn_h * valid_exp.float()
        attn_h = attn_h / attn_h.sum(dim=-1, keepdim=True).clamp(min=1e-12)
        attn_h = self.attention_dropout(attn_h)  # (B, H, C, S)

        # per-head message: (B, H, C, Dh)
        msg_h = torch.matmul(attn_h, vh)

        # target-conditioned head gate: (B, C, H)
        ...
        # weighted sum over heads: (B, C, Dh)
        # head_weight: (B, C, H) -> (B, H, C) -> (B, H, C, 1)
        hw = head_weight.transpose(1, 2).unsqueeze(-1)  # (B, H, C, 1)
        msg = (msg_h * hw).sum(dim=1)  # (B, C, Dh)

        # project merged head output back to hidden_dim
        msg = self.head_output_projs[layer_idx](msg)  # (B, C, hidden_dim)

        # 保存诊断信息（用 head 0 的 attention 作代表）
        attn0 = attn_h[:, 0]  # (B, C, S)
