...
def main() -> None:
    ...
    model = InferenceDrivenGNN(
        ...
        head_aggregation=str(model_cfg.get("head_aggregation", "dynamic_attention")),
        head_gate_temperature=float(model_cfg.get("head_gate_temperature", 0.5)),
        head_gate_bias_init=float(model_cfg.get("head_gate_bias_init", 0.0)),
    ).to(device)
    print(f"  模型架构: {architecture}")
    print(f"  n_heads: {int(model_cfg.get('n_heads', 1))} ({str(model_cfg.get('head_aggregation','dynamic_attention'))})")
