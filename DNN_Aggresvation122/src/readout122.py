# -*- coding: utf-8 -*-
"""（122 号复制自 115 号 readout.py，仅把单位阵改为连续内存以便在 CPU 上运行；数值结果相同）
...
def ridge_predict(F_tr, Y_tr, F_te, fit_idx, val_idx, sd_floor=1e-8, drop_const=0.0, cv="holdout", folds=None,
                  clip_te=False, winsor=None, clip_y=False, alphas=None):
    ...
    if winsor is not None:   # 122 号：标准化特征缩尾到 ±winsor（训练、测试同一规则，只用训练统计量）
        T, E = T.clamp(-winsor, winsor), E.clamp(-winsor, winsor)
    if clip_te:   # 测试特征截断到训练集取值范围，防止尖峰输入把线性读出外推爆掉
        E = torch.minimum(torch.maximum(E, T.amin(1, keepdim=True)), T.amax(1, keepdim=True))
    AL = ALPHAS if alphas is None else alphas
    B, n, d = T.shape; C = Y_tr.shape[1]
    eye = torch.eye(d, device=T.device, dtype=T.dtype).expand(B, d, d).contiguous()   # 122 号：CPU 的 LAPACK 需要连续内存
    ...
    if clip_y:   # 122 号：预测截断到训练目标的取值范围
