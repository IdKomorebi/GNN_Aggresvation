"""
端到端推断驱动的GNN模型（DNN_Aggresvation4）。

子项目4保留子项目3的v2 GCN作为可复现基线，同时新增GAT化消息传递：
- GCN模式：使用相关性融合后的固定行归一化邻接矩阵做均匀聚合
- GAT模式：在固定边掩码内，对每个目标节点学习邻居注意力权重

两种模式都保留：
- 5种相关性指标的可学习融合权重 alpha
- Confidential节点可学习嵌入
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        # 可学习的相关性融合权重 beta -> alpha = softmax(beta)。
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        A = self.compute_adjacency(row_normalize=self.architecture == "gcn")
