"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation9）。

子项目9融合DNN6的强target-bilinear attention与DNN8的目标级关系先验：
- General-General: 使用一组全局alpha_G构造固定相关性边权，并做加权GCN聚合
- Confidential-Source: 每个Confidential目标学习一组alpha_c，得到目标专属关系先验p(c,j)
- 动态项使用DNN6式target bilinear score，并通过gate与p(c,j)融合
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        self.relation_bias_layers = nn.ModuleList()
        ...
        for _ in range(num_layers):
            ...
            self.relation_bias_layers.append(nn.Linear(self.n_metrics, 1))
            ...
            nn.init.constant_(self.gate_r_layers[-1].bias, float(gate_bias_init))
    ...
    def _confidential_messages(
        ...
        relation_bias = self.relation_bias_layers[layer_idx](rel).squeeze(-1)
        dynamic_score = dynamic_score + relation_bias.unsqueeze(0)
