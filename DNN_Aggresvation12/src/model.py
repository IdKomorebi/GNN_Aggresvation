"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation12）。

子项目12在DNN11基础上继续改General-General alpha：
- General-General: 每个General target节点学习一组alpha_i^G，同一target的入边共享
- Confidential-Source: 每个Confidential目标学习一组alpha_c，得到目标专属关系先验p(c,j)
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        # 每个General target节点一组相关性融合权重；同一target的所有General入边共享。
        self.beta_general = nn.Parameter(
            torch.zeros(self.n_general, self.n_metrics, dtype=torch.float32)
        )
        # 每个Confidential目标一组相关性融合权重。
        self.beta_confidential = nn.Parameter(
            torch.zeros(self.n_confidential, self.n_metrics, dtype=torch.float32)
        )
        if alpha_init_std > 0:
        ...
        self.general_query_layers = nn.ModuleList()
        self.general_key_layers = nn.ModuleList()
        self.general_value_layers = nn.ModuleList()
        self.general_gate_q_layers = nn.ModuleList()
        self.general_gate_k_layers = nn.ModuleList()
        self.general_gate_p_layers = nn.ModuleList()
        self.general_prior_scales = nn.ParameterList()
        ...
        for _ in range(num_layers):
            ...
            self.general_query_layers.append(nn.Linear(hidden_dim, self.attention_dim, bias=False))
            self.general_key_layers.append(nn.Linear(hidden_dim, self.attention_dim, bias=False))
            self.general_value_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
            self.general_gate_q_layers.append(nn.Linear(self.attention_dim, 1, bias=False))
            self.general_gate_k_layers.append(nn.Linear(self.attention_dim, 1, bias=False))
            self.general_gate_p_layers.append(nn.Linear(1, 1, bias=True))
            nn.init.constant_(self.general_gate_p_layers[-1].bias, float(gate_bias_init))
            self.general_prior_scales.append(nn.Parameter(torch.tensor(float(prior_scale_init))))
        ...
        self.last_general_attention: torch.Tensor | None = None
        self.last_general_gate: torch.Tensor | None = None
        self.last_general_prior: torch.Tensor | None = None
    ...
    def get_alpha(self) -> torch.Tensor:
        """兼容训练日志：返回General target alpha的均值。"""
        return self.get_alpha_general().mean(dim=0)

    def get_alpha_general(self) -> torch.Tensor:
        return torch.softmax(self.beta_general / self.edge_alpha_temperature, dim=-1)
    ...
    def compute_general_adjacency(self, row_normalize: bool = True) -> torch.Tensor:
        """兼容旧诊断：返回General-General先验边权。"""
        A = self.compute_general_prior()
        if not row_normalize:
            return A
        row_sum = A.sum(dim=1, keepdim=True).clamp(min=1e-12)
        return A / row_sum

    def compute_general_prior(self) -> torch.Tensor:
        """返回形状(N,N)的General-General关系先验p^G_{i,j}。"""
        alpha_g = self.get_alpha_general()
        rel_gg = self.metric_tensor[: self.n_general, : self.n_general, :]
        A_gg = torch.sum(rel_gg * alpha_g.unsqueeze(1), dim=-1)
        A_raw = torch.zeros(
            self.n_nodes,
            self.n_nodes,
        ...
        A_raw[: self.n_general, : self.n_general] = A_gg
    ...
    def _general_messages(
        ...
        """General target对General source的gated log-prior attention。"""
        B = h.shape[0]
        h_g = h[:, : self.n_general, :]

        q = self.general_query_layers[layer_idx](h_g)
        k = self.general_key_layers[layer_idx](h_g)
        v = self.general_value_layers[layer_idx](h_g)

        prior_g = prior[: self.n_general, : self.n_general]
        prior_log = torch.log(prior_g.clamp(min=self.prior_log_eps))
        ...
        if self.architecture == "hybrid_bilinear_prior_only":
            ...
            scale = torch.nn.functional.softplus(self.general_prior_scales[layer_idx])
            score = dynamic_score + scale * prior_score
            gate = torch.ones_like(score)
        else:
            gate_logits = (
                self.general_gate_q_layers[layer_idx](q)
                + self.general_gate_k_layers[layer_idx](k).transpose(1, 2)
                + self.general_gate_p_layers[layer_idx](prior_score.unsqueeze(-1)).squeeze(-1)
            )
        ...
        self.last_general_attention = attention.detach()
        self.last_general_gate = gate.detach()
        self.last_general_prior = prior_g.detach()
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        general_prior = self.compute_general_prior()
        ...
        for layer_idx, (self_layer, gen_layer, conf_layer, bn) in enumerate(
            ...
            general_messages = self._general_messages(h, layer_idx, general_prior)
    ...
    def get_mean_attention(self, node_values: torch.Tensor) -> np.ndarray | None:
        ...
        with torch.no_grad():
            ...
            if self.last_general_attention is not None:
                attention[: self.n_general, : self.n_general] = (
                    self.last_general_attention.mean(dim=0).cpu().numpy()
                )
        if was_training:
            self.train()
        return attention
