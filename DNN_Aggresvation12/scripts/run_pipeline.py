"""
DNN_Aggresvation12 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（KNN+阈值边掩码）
4. 训练General节点也使用target-specific alpha attention的hybrid gated prior GAT
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN12混合聚合的alpha、prior、gate诊断。"""
    ...
    alpha_g_mean = alpha_g.mean(axis=0)
    pd.DataFrame({"metric": metrics, "alpha_general": alpha_g_mean}).to_csv(
        diag_dir / "alpha_general.csv", index=False
    )
    alpha_g_df = pd.DataFrame(alpha_g, columns=metrics)
    alpha_g_df.insert(0, "target_field", data_info["general"])
    alpha_g_df.insert(0, "node_type", "general")
    alpha_g_df.to_csv(diag_dir / "alpha_general_by_target.csv", index=False)

    alpha_c = model.get_alpha_confidential().detach().cpu().numpy()
    alpha_c_df = pd.DataFrame(alpha_c, columns=metrics)
    alpha_c_df.insert(0, "target_field", conf_names)
    alpha_c_df.insert(0, "node_type", "confidential")
    alpha_c_df.to_csv(diag_dir / "alpha_confidential_by_target.csv", index=False)

    all_alpha_df = pd.concat([alpha_g_df, alpha_c_df], ignore_index=True)
    dominant_idx = all_alpha_df[metrics].to_numpy().argmax(axis=1)
    all_alpha_df["dominant_metric"] = [metrics[idx] for idx in dominant_idx]
    all_alpha_df["dominant_alpha"] = all_alpha_df[metrics].max(axis=1)
    all_alpha_df.to_csv(diag_dir / "alpha_by_all_nodes.csv", index=False)
    ...
    try:
        ...
        fig_h = max(10.0, 0.26 * len(all_alpha_df))
        fig, ax = plt.subplots(figsize=(13, fig_h))
        y = np.arange(len(all_alpha_df))
        left = np.zeros(len(all_alpha_df))
        colors = ["#4e79a7", "#f28e2b", "#e15759", "#76b7b2", "#59a14f"]
        labels = [
            f"{row.node_type[:1].upper()} | {row.target_field}"
            for row in all_alpha_df.itertuples(index=False)
        ]
        for metric_name, color in zip(metrics, colors):
            values = all_alpha_df[metric_name].to_numpy()
        ...
        ax.set_title("Node-specific correlation metric weights (G=general, C=confidential)")
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.035), ncol=len(metrics), fontsize=8)
        ax.grid(True, axis="x", alpha=0.25)
        fig.tight_layout()
        fig.savefig(diag_dir / "alpha_by_all_nodes_stacked.png", dpi=220, bbox_inches="tight")
        plt.close(fig)
    except ImportError:
        pass
    ...
    for conf_pos, target_idx in enumerate(conf_indices):
        ...
        for source_idx, source_name in enumerate(field_names):
            ...
            row = {
                ...
                "source_type": "confidential" if source_idx in set(conf_indices) else "general",
    ...
    print(f"  DNN12混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation12: node-specific-alpha general-attention hybrid gated prior GAT 推断驱动敏感度评估流水线"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation12: node-specific-alpha general-attention hybrid gated prior GAT 推断驱动敏感度评估")
    print("=" * 60)
    ...
    summary = {
        ...
        "alpha_general_mean": {
            m: float(v)
            for m, v in zip(metrics, model.get_alpha_general().detach().cpu().numpy().mean(axis=0))
        },
        "alpha_general_by_target": {
            target: {
                metric: float(value)
                for metric, value in zip(
                    metrics,
                    model.get_alpha_general().detach().cpu().numpy()[idx],
                )
            }
            for idx, target in enumerate(data_info["general"])
        },
        "alpha_confidential_by_target": {
            target: {
                metric: float(value)
                for metric, value in zip(
                    metrics,
                    model.get_alpha_confidential().detach().cpu().numpy()[idx],
                )
            }
            for idx, target in enumerate(data_info["confidential"])
        },
