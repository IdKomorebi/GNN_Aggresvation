"""
DNN_Aggresvation19 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（固定KNN或自适应阈值-KNN边掩码）
4. 训练带difficulty-aware target weighting的hybrid gated prior GAT
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN19混合聚合的alpha、prior、gate诊断。"""
    ...
    print(f"  DNN19混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation19: difficulty-aware target loss weighting pipeline"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation19: difficulty-aware target weighting")
    print("=" * 60)
