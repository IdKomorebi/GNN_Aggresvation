...
def load_rescan(order: int, kind: str, source: str | None = None) -> pd.DataFrame:
    """source=None 读 90 号 full 字典的重扫；否则读本号 scan_order5 的产出前缀。"""
    if source:
        fs = sorted(glob.glob(str(ROOT / f"outputs/{source}_s*of*.parquet")))
    else:
        fs = sorted(glob.glob(str(REPO / f"DNN_Aggresvation90/outputs/"
                                   f"rescan_o{order}_{kind}_audit_s*.parquet")))
...
def main() -> None:
    ...
    # 断点续跑：跨**所有**分片文件识别已完成，这样中途改分片数不会重复劳动。
    # 同批启动的分片互不重叠（i::nshard），故只与"已停止的旧分片"比对，无竞态。
    done = set()
    for f in glob.glob(str(ROOT / f"outputs/certify_o{args.order}_s*of*.csv")):
        try:
