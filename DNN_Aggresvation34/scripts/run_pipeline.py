...
def main() -> None:
    ...
    agg_short = "dynamic" if head_aggregation == "dynamic_attention" else "static"
    if single_target:
        model_variant = f"single_{single_target}"
    elif n_heads > 1:
        # 多头时区分第二级聚合机制：dynamic_{n}head / static_{n}head
        model_variant = f"{agg_short}_{n_heads}head"
    else:
        # 单头时两种聚合等价
        model_variant = "baseline_1head"
