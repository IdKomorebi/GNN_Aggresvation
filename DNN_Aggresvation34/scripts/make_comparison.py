#!/usr/bin/env python3
"""DNN34 跨 run 汇总：对比动态 head 注意力 vs 静态 head gate vs 单头基线。
...
生成：
  - outputs/dynamic_vs_static_comparison.csv
  - outputs/dynamic_vs_static.png
...
def main() -> None:
    ...
    table["dyn4_minus_stat4"] = table["dynamic_4head"] - table["static_4head"]
    table["dyn8_minus_stat8"] = table["dynamic_8head"] - table["static_8head"]
    table["dyn8_minus_base"] = table["dynamic_8head"] - table["baseline_1head"]
    table["dyn8_minus_single"] = table["dynamic_8head"] - table["single_r2"]

    out_csv = OUT / "dynamic_vs_static_comparison.csv"
    ...
    print("\n===== 动态 vs 静态（用户思路 vs GLM）=====")
    print(f"  4head  dynamic-static: {stat('dyn4_minus_stat4')}")
    print(f"  8head  dynamic-static: {stat('dyn8_minus_stat8')}")
    print(f"  8head dynamic-baseline: {stat('dyn8_minus_base')}")
    print(f"  8head dynamic-single  : {stat('dyn8_minus_single')}")

    # ---- 柱状图：每字段 single(上限) / baseline / static_8 / dynamic_8 ----
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        bars = [
            ...
            ("baseline_1head", "baseline_1head", "#4c72b0"),
            ("static_8head (GLM)", "static_8head", "#dd8452"),
            ("dynamic_8head (user)", "dynamic_8head", "#55a868"),
        ]
        ...
        ax.set_title(
            "DNN34: dynamic head-attention (user) vs static gate (GLM) vs "
            "baseline, with single upper bound"
        )
        ax.legend(ncol=2)
        ax.grid(axis="y", alpha=0.3)
        fig.tight_layout()
        fig.savefig(OUT / "dynamic_vs_static.png", dpi=130)
        print(f"\n图已保存: {OUT / 'dynamic_vs_static.png'}")
