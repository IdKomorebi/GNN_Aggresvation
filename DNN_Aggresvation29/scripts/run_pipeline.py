"""
DNN_Aggresvation29 主流水线脚本。

在 DNN25 流水线基础上：
- 支持 bipartite 二分传播（切断 Confidential-Confidential 边）
...
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（固定KNN或自适应阈值-KNN边掩码，可选 bipartite）
4. 训练 hybrid gated prior GAT（支持 staged training + temporal window）
...
def main() -> None:
    ...
    # 创建运行目录：outputs/<model_name>/<loss_mode>/<timestamp>/
    model_cfg = cfg.get("model", {})
    model_name = "bipartite" if bool(model_cfg.get("bipartite", False)) else "baseline"
    loss_excluded = cfg.get("training", {}).get("loss_exclude_confidential", [])
    loss_mode = "maskloss" if loss_excluded else "allloss"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = output_root / model_name / loss_mode / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation29: bipartite vs baseline")
    print("=" * 60)
