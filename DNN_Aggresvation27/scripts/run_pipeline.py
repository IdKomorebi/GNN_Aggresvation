"""
DNN_Aggresvation27 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（固定KNN或自适应阈值-KNN边掩码）
4. 训练multi-head confidential attention的hybrid gated prior GAT
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation27: multi-head attention pipeline"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation27: multi-head attention")
    print("=" * 60)
    ...
    print(f"  attention_heads: {int(model_cfg.get('attention_heads', 1))}")
