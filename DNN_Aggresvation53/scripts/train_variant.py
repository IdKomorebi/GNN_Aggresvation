...
def main() -> None:
    ...
    model = InferenceDrivenGNN(
        ...
        gat_variant=str(m.get("gat_variant", "transformer")),
    ).to(device)

    print(f"[{ENCODER_TAG[args.encoder]}/{args.mode}] encoder={args.encoder} "
          f"mode={args.mode} gat_variant={m.get('gat_variant','transformer')}")
