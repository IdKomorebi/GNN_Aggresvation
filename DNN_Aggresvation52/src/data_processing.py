...
def prepare_data(cfg: dict) -> dict:
    ...
    支持单目标模式：当 cfg["fields"]["single_target"] 存在时，
    只保留该 1 个 Confidential 字段，其余 11 个从节点集中移除。
    图结构只包含 General-General + General→该 Confidential。
    """
    csv_path = cfg["dataset"]["csv_path"]
    drop_columns = cfg["fields"].get("drop_columns", [])
    original_confidential_columns = list(cfg["fields"]["confidential"])
    confidential_columns = list(original_confidential_columns)
    ...
    if single_target:
        if single_target not in original_confidential_columns:
            raise ValueError(f"single_target '{single_target}' 不在 confidential 列表中")
        # 信息集修正：single 模式只能看到原始 General 字段，不能把其它
        # confidential 字段错误地归入 General。其它 11 个 confidential 直接移除。
        other_confidential = [
            c for c in original_confidential_columns if c != single_target
        ]
        drop_columns = list(drop_columns) + other_confidential
        confidential_columns = [single_target]
        print(f"  单目标模式: 只保留 target={single_target}")
        print(f"  单目标信息集修正: 移除其它Confidential字段 {len(other_confidential)} 个")
