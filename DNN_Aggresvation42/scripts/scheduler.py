#!/usr/bin/env python3
"""DNN42 调参调度器：4 GPU 并行跑 configs/ 下的全部调参配置。
...
D42 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation42")
PY = "/data1/duhaocun/miniconda3/envs/Pytorch310_codex/bin/python"
GPUS = [0, 1, 2, 3]
LOG_DIR = D42 / "logs"
LOG_DIR.mkdir(exist_ok=True)

configs = sorted((D42 / "configs").glob("*.yaml"))

queue = deque(configs)
gpu_pool = list(GPUS)
running = {}  # gpu -> (proc, config_name)

print(f"DNN42 调参调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D42 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
