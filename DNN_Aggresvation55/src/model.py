...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        unified_aggregation: str = "none",
        gat_variant: str = "transformer",
        conf_gat_variant: str = "transformer",
    ) -> None:
        super().__init__()
        ...
        # 统一聚合（DNN52）：2×3 网格 = {gcn, gat} × {无先验, 全局静态先验, 全局动态先验}。
        # gcn = 无 q·k 内容注意力（结构化聚合 raw h）；gat = q·k 注意力聚合 value(h)。
        #   gcn_noprior : 均匀邻接(行归一)，不用相关系数也不用注意力。
        #   gcn_static  : 相关性先验加权的固定邻接(全局静态 alpha)。
        #   gcn_dynamic : 先验邻接 × 逐样本 sigmoid 门控(调制每条先验边强度)，行归一；无 q·k。
        #   gat_noprior : 纯 q·k 注意力，无先验。
        #   gat_static  : q·k + 单一全局 scale × log(先验)（no_gate）。
        #   gat_dynamic : 逐对动态 gate 混合 q·k 与 log(先验)。
        # 图结构不变：所有节点都只从其 general 邻居聚合。
        self.unified_aggregation = str(unified_aggregation)
        if self.unified_aggregation != "none":
            valid = {"gcn_noprior", "gcn_static", "gcn_dynamic",
                     "gat_noprior", "gat_static", "gat_dynamic"}
            if self.unified_aggregation not in valid:
                raise ValueError(f"Unknown unified_aggregation: {self.unified_aggregation}")
            support = np.array(edge_mask, dtype=np.float32).copy()
            support[:, n_general:] = 0.0          # 源只能是 general（保持图结构与分裂设计一致）
            np.fill_diagonal(support, 0.0)
            self.register_buffer("agg_support", torch.as_tensor(support, dtype=torch.float32))
        # 诊断用：forward 时记录每层 unified 注意力 / 门控（均值 over batch）。
        self._uni_attn: list[torch.Tensor] = []
        self._uni_gate: list[torch.Tensor] = []
        ...
        if self.input_encoder == "mlp_deep":  # 最强：3 层 MLP
        ...
        elif self.input_encoder == "mlp":  # 强：2 层 MLP（当前默认）
            self.input_proj = nn.Sequential(
        ...
        elif self.input_encoder == "linear":  # 中：单线性 + ReLU
            self.input_proj = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                nn.ReLU(),
            )
        elif self.input_encoder == "linear_noact":  # 弱：单线性，无激活
            self.input_proj = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
            )
        else:
        ...
        # 经典 GAT（DNN53）：共享 W 得 z=Wh（K=V=Wh），加性注意力 LeakyReLU(a^T[z_i‖z_j])。
        # DNN55：conf_gat_variant 让分裂设计的 general→confidential 半边也可用经典 GAT。
        self.gat_variant = str(gat_variant)
        self.conf_gat_variant = str(conf_gat_variant)
        if self.gat_variant == "classic" or self.conf_gat_variant == "classic":
            self.gat_W_layers = nn.ModuleList()       # z = W h（既当 K 又当 V）
            self.gat_a_src_layers = nn.ModuleList()   # a_l^T z_i（目标侧）
            self.gat_a_dst_layers = nn.ModuleList()   # a_r^T z_j（源侧）
            self.gat_gate_src_layers = nn.ModuleList()  # 动态门控用（仅 gat_dynamic）
            self.gat_gate_dst_layers = nn.ModuleList()
            self.gat_leakyrelu = nn.LeakyReLU(0.2)
        ...
        for _ in range(num_layers):
            ...
            if self.gat_variant == "classic" or self.conf_gat_variant == "classic":
                self.gat_W_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
                self.gat_a_src_layers.append(nn.Linear(hidden_dim, 1, bias=False))
                self.gat_a_dst_layers.append(nn.Linear(hidden_dim, 1, bias=False))
                self.gat_gate_src_layers.append(nn.Linear(hidden_dim, 1, bias=False))
                self.gat_gate_dst_layers.append(nn.Linear(hidden_dim, 1, bias=False))
    ...
    def _confidential_messages(
        ...
        classic = self.conf_gat_variant == "classic"
        if classic:
            # 经典加性 GAT：共享 W 得 z（K=V=Wh），目标侧带 target 身份
            z_conf = self.gat_W_layers[layer_idx](h_conf + target_id)   # (B,C,H)
            z_src = self.gat_W_layers[layer_idx](h_src)                 # (B,G,H) = K = V
            v = z_src
            e_src = self.gat_a_src_layers[layer_idx](z_conf)           # (B,C,1) 目标侧
            e_dst = self.gat_a_dst_layers[layer_idx](z_src)            # (B,G,1) 源侧
            dynamic_score = self.gat_leakyrelu(e_src + e_dst.transpose(1, 2))  # (B,C,G)
        else:
            # transformer：独立 Q/K/V，q·k/√d
            q = (
        ...
        if self.architecture == "hybrid_bilinear_prior_only":
            ...
            if classic:
                gate_logits = (
                    self.gat_gate_src_layers[layer_idx](z_conf)
                    + self.gat_gate_dst_layers[layer_idx](z_src).transpose(1, 2)
    ...
    def _full_prior(self) -> torch.Tensor:
        """(N,N) 全局静态相关性先验：共享 alpha_general 融合相关性张量，× support。"""
        alpha_g = self.get_alpha_general()
        return torch.einsum("ijk,k->ij", self.metric_tensor, alpha_g) * self.agg_support

    def _unified_messages(self, h: torch.Tensor, layer_idx: int) -> torch.Tensor:
        """统一聚合（DNN52）：2×3 网格 {gcn,gat}×{无/静态/动态先验}，源限定 general。"""
        mode = self.unified_aggregation
        support = self.agg_support                                  # (N,N) 二值，仅 general 源
        B = h.shape[0]
        valid = support > 0

        # ---------- GCN 系：无 q·k 注意力，行归一加权邻接聚合 raw h ----------
        if mode.startswith("gcn"):
            if mode == "gcn_noprior":
                A = support.unsqueeze(0).expand(B, -1, -1)
            elif mode == "gcn_static":
                A = self._full_prior().unsqueeze(0).expand(B, -1, -1)
            else:  # gcn_dynamic：先验边 × 逐样本 sigmoid 门控（无 q·k 内容注意力）
                P = self._full_prior()
                prior_log = torch.log(P.clamp(min=self.prior_log_eps))
                prior_score = prior_log.unsqueeze(0).expand(B, -1, -1)
                q = self.query_hidden_layers[layer_idx](h)
                ...
                gate = torch.sigmoid(gate_logits) * valid.unsqueeze(0).float()
                self._uni_gate.append(gate.detach().mean(0))
                A = gate * P.unsqueeze(0)                          # 动态调制每条先验边强度
            A = A / A.sum(dim=-1, keepdim=True).clamp(min=1e-12)
            return torch.bmm(A, h)                                 # (B,N,H)

        # ---------- GAT 系：注意力后聚合 value ----------
        classic = self.gat_variant == "classic"
        if classic:
            # 经典 GAT：z=Wh（K=V=Wh），加性注意力 LeakyReLU(a_l^T z_i + a_r^T z_j)
            z = self.gat_W_layers[layer_idx](h)                   # (B,N,H)  = Wh = K = V
            v = z
            e_src = self.gat_a_src_layers[layer_idx](z)           # (B,N,1) 目标 i 侧
            e_dst = self.gat_a_dst_layers[layer_idx](z)           # (B,N,1) 源 j 侧
            score = self.gat_leakyrelu(e_src + e_dst.transpose(1, 2))  # (B,N,N)
        else:
            # transformer：q·k 内容注意力，独立 Q/K/V
            q = self.query_hidden_layers[layer_idx](h)            # (B,N,ad)
            k = self.key_layers[layer_idx](h)                     # (B,N,ad)
            v = self.value_layers[layer_idx](h)                   # (B,N,H)
            score = torch.matmul(q, k.transpose(1, 2)) / math.sqrt(self.attention_dim)

        if mode in ("gat_static", "gat_dynamic"):
            prior_log = torch.log(self._full_prior().clamp(min=self.prior_log_eps))
            prior_score = prior_log.unsqueeze(0).expand(B, -1, -1)
            scale = torch.nn.functional.softplus(self.prior_scales[layer_idx])
            if mode == "gat_static":
                # 全局静态(no_gate)：注意力 + 单一全局 scale × log(先验)
                score = score + scale * prior_score
            else:
                # 全局动态：逐对 gate 在注意力与先验间混合
                if classic:
                    gate_logits = (
                        self.gat_gate_src_layers[layer_idx](z)
                        + self.gat_gate_dst_layers[layer_idx](z).transpose(1, 2)
                        + self.gate_p_layers[layer_idx](prior_score.unsqueeze(-1)).squeeze(-1)
                    )
                else:
                    gate_logits = (
                ...
                gate = torch.sigmoid(gate_logits)
                self._uni_gate.append((gate * valid.unsqueeze(0).float()).detach().mean(0))
                score = gate * score + (1.0 - gate) * scale * prior_score
        ...
        attention = attention * valid.unsqueeze(0).float()
        attention = attention / attention.sum(dim=-1, keepdim=True).clamp(min=1e-12)
        self._uni_attn.append(attention.detach().mean(0))         # 诊断：每层注意力
        attention = self.attention_dropout(attention)
        return torch.bmm(attention, v)                            # (B,N,H)

    def get_unified_attention(self) -> torch.Tensor | None:
        """跨层平均的 unified 注意力 (N,N)；仅 gat 系有。需先跑一次 forward。"""
        if not self._uni_attn:
            return None
        return torch.stack(self._uni_attn, 0).mean(0)

    def get_unified_gate(self) -> torch.Tensor | None:
        """跨层平均的 unified 门控 (N,N)；仅 *_dynamic 有。需先跑一次 forward。"""
        if not self._uni_gate:
            return None
        return torch.stack(self._uni_gate, 0).mean(0)
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        unified = self.unified_aggregation != "none"
        if unified:
            self._uni_attn = []   # 重置诊断缓存（每层 mean-over-batch 注意力/门控）
            self._uni_gate = []
            A_general = None
            conf_prior = None
        else:
        ...
        for layer_idx, (self_layer, gen_layer, conf_layer, bn) in enumerate(
            ...
            if self.unified_aggregation != "none":
                # 统一聚合消融：整张图同一种聚合方式
                messages = self._unified_messages(h, layer_idx)
                self.layer_attentions.append(None)
            else:
