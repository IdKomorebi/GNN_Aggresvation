"""
DNN_Aggresvation32 主流水线脚本。

在 DNN31 最优配置（bipartite + shuffle + nostaged + allloss）基础上，
做多目标 vs 单目标推断对照：
- multi：1 个模型同时推断全部 12 个 Confidential 字段
- single：12 个独立模型，每次只推断 1 个 Confidential 字段（图结构只含
  General-General + General→该 Confidential）

输出目录结构：
- outputs/multi/<timestamp>/
- outputs/single/<target_name>/<timestamp>/

目的：验证多目标联合推断是否会降低每个目标的预测质量。
"""
...
def _save_r2_comparison_table(
    ...
    headers = ["Confidential field", "DNN probe R2", "Model test R2", "Model - Probe"]
    xs = [0.02, 0.58, 0.75, 0.92]
    y_top = 0.90
...
def main() -> None:
    ...
    # 创建运行目录（DNN42 调参：用 experiment.name 命名，每个配置独立可对应）
    single_target = cfg.get("fields", {}).get("single_target", None)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = output_root / "tuning" / experiment_name / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation40: window_size=1 fixed-info GNN experiment")
    print("=" * 60)
    print(f"  实验名称: {experiment_name}")
    print(f"  模式: {'single (' + single_target + ')' if single_target else 'multi (12 targets)'}")
    ...
    # 字段缺失：按 field_keep_frac 固定选取保留的 general 字段（seed 123）
    field_keep_frac = float(cfg.get("fields", {}).get("field_keep_frac", 1.0))
    field_keep_mask = None
    if field_keep_frac < 1.0:
        _ng = data_info["n_general"]
        _keep = max(1, int(round(_ng * field_keep_frac)))
        _sel = np.random.default_rng(123).choice(_ng, _keep, replace=False)
        field_keep_mask = np.zeros(_ng, dtype=np.float32); field_keep_mask[_sel] = 1.0
        print(f"  [字段缺失] 保留 {_keep}/{_ng} 个 general 字段 (frac={field_keep_frac})")
    model = InferenceDrivenGNN(
        metric_tensor=metric_tensor,
        edge_mask=edge_mask,
        n_nodes=data_info["n_nodes"],
        ...
        bipartite=use_bipartite,
        use_input_gate=bool(model_cfg.get("use_input_gate", False)),
        field_keep_mask=field_keep_mask,
    ).to(device)
