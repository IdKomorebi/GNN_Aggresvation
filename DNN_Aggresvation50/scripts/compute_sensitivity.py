...
def load_model_and_test(cfg_path: Path, model_pt: Path, device: torch.device):
    ...
    g = cfg.get("graph", {})
    m = cfg.get("model", {})
    # 字段缺失 mask（与训练同 seed/逻辑，保证一致）
    field_keep_frac = float(cfg.get("fields", {}).get("field_keep_frac", 1.0))
    field_keep_mask = None
    if field_keep_frac < 1.0:
        ngg = data_info["n_general"]
        kp = max(1, int(round(ngg * field_keep_frac)))
        sl = np.random.default_rng(123).choice(ngg, kp, replace=False)
        field_keep_mask = np.zeros(ngg, dtype=np.float32); field_keep_mask[sl] = 1.0
    data_info["field_keep_mask"] = field_keep_mask
