...
def main():
    ...
    res = {"k": args.k, "struct": args.struct, "mean_r2": float(r2.mean()),
           "per_conf_r2": {di["confidential"][j]: float(r2[j]) for j in range(len(ci))}}
    (ROOT / "outputs/retrain" / f"{args.struct}_k{args.k}.json").write_text(json.dumps(res, indent=2, ensure_ascii=False))
    print(f"[{args.struct} k={args.k}] 12-conf 平均 R² = {r2.mean():.4f}")
