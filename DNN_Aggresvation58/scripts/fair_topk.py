...
def main():
    ...
    def make_leak(kind, mdl=None):
        if kind == "dnn":
            ...
            return lambda S: float((aw * dnn_leak(S)).sum())
        ac = gcn_leak_vec(mdl, set(range(nG))); aw = ac / ac.sum()
        return lambda S: float((aw * gcn_leak_vec(mdl, S)).sum())
    ...
    oracle = lambda S: float((aw_m * gcn_leak_vec(gcn_masked, S)).sum())
    ...
    out = ROOT / "outputs"
    pd.DataFrame({"k": ks, **curves}).to_csv(out / "fair_topk.csv", index=False)
    ...
    ax.set_ylabel("TRUE leakage v(top-k) — all measured by the SAME faithful (masked) oracle")
    ax.set_title("DNN58 FAIR top-k: 5 rankings evaluated on one common faithful v(S)")
    ax.legend(fontsize=8); ax.grid(alpha=0.3)
    fig.tight_layout(); fig.savefig(out / "fair_topk.png", dpi=140); plt.close(fig)
