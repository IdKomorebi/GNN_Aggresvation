...
def sample_mask(batch: int, n_general: int, rng: np.random.RandomState) -> np.ndarray:
    ...
    return sample_mask_banded(batch, n_general, rng, 1, n_general)


def sample_mask_banded(batch: int, n_general: int, rng: np.random.RandomState,
                       size_lo: int, size_hi: int) -> np.ndarray:
    """分带掩码：保留数 k~U{size_lo..size_hi}，再均匀抽 k 个可见字段。
    size_lo=1,size_hi=nG 时退化为全尺寸 uniform（与原 sample_mask 等价）。"""
    m = np.zeros((batch, n_general), dtype=np.float32)
    ks = rng.randint(size_lo, size_hi + 1, size=batch)
