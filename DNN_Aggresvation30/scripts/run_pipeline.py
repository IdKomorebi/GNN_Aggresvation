"""
DNN_Aggresvation30 主流水线脚本。

在 DNN29 bipartite 主干基础上：
- 支持 split_mode: temporal | shuffle 两种 train/test 切分方式
- 输出目录结构改为 outputs/<split_mode>/<loss_mode>/<timestamp>/
  其中 split_mode ∈ {temporal, shuffle}，loss_mode ∈ {maskloss, allloss}
- DNN30 固定使用 bipartite=True（DNN29 已证明 bipartite 更合理）

完整流程：
1. 数据加载与预处理（temporal 或 shuffle 切分）
2. 计算5种相关性指标张量
3. 构建图结构（bipartite，切断 conf-conf 边）
4. 训练 hybrid gated prior GAT（staged training + temporal window）
...
def main() -> None:
    ...
    # 创建运行目录：outputs/<split_mode>/<loss_mode>/<timestamp>/
    ...
    run_dir = output_root / split_mode / loss_mode / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation30: split_mode comparison (bipartite fixed)")
    print("=" * 60)
    print(f"  实验名称: {experiment_name}")
    print(f"  切分方式: {split_mode}")
