"""
DNN_Aggresvation17 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（固定KNN或自适应阈值-KNN边掩码）
4. 训练Confidential入边edge-specific alpha的hybrid gated prior GAT
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN17混合聚合的alpha、prior、gate诊断。"""
    ...
    print(f"  DNN17混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation17: target residual expert pipeline"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation17: target-specific residual expert")
    print("=" * 60)
    ...
    model = InferenceDrivenGNN(
        ...
        use_target_residual_expert=bool(model_cfg.get("use_target_residual_expert", False)),
        residual_init_scale=float(model_cfg.get("residual_init_scale", 0.1)),
    ).to(device)
    ...
    print(f"  use_target_residual_expert: {bool(model_cfg.get('use_target_residual_expert', False))}")
    print(f"  residual_init_scale: {float(model_cfg.get('residual_init_scale', 0.1))}")
