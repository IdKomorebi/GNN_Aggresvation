"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation17）。

子项目17回到General-General fixed weighted GCN主线，并增加target-specific residual expert：
...
- 默认使用target-specific output heads，并可叠加target-specific residual expert
"""
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        use_target_residual_expert: bool = False,
        ...
        self.use_target_residual_expert = bool(use_target_residual_expert)
        ...
        init_scale = min(max(float(residual_init_scale), 1e-4), 1.0 - 1e-4)
        init_logit = math.log(init_scale / (1.0 - init_scale))
        self.residual_scale_logits = nn.Parameter(
            torch.full((self.n_confidential,), init_logit, dtype=torch.float32)
        )
    ...
    def _target_residual(
        ...
        """Target-specific residual expert from prior-weighted source context."""
        weights = prior / prior.sum(dim=1, keepdim=True).clamp(min=1e-12)
        context = torch.bmm(weights.unsqueeze(0).expand(h.shape[0], -1, -1), h)
        expert_inputs = torch.cat([h_conf, context], dim=-1)
        residuals = [
            expert(expert_inputs[:, target_pos, :])
            for target_pos, expert in enumerate(self.residual_experts)
        ]
        residual = torch.cat(residuals, dim=1)
        scale = torch.sigmoid(self.residual_scale_logits).view(1, -1)
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_specific_heads:
            ...
            base_output = torch.cat(outputs, dim=1)
        else:
            base_output = self.output_head(h_conf).squeeze(-1)

        if not self.use_target_residual_expert:
            return base_output
        return base_output + self._target_residual(h, h_conf, conf_prior)
