...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{model_arch_zh.png}
  \caption{可见性感知的图消息传递与目标读出。节点输入同时编码字段值和可见位；不可见邻居被移出注意力归一化，每个机密目标再由独立查询向量读取相应的字段子图表示。}
...
本文实验图表均由项目既有 CSV/JSON 产物重新生成，未手工填写绘图数据；图\ref{fig:model-arch}为方法结构示意图。关键来源如下：
\begin{itemize}
