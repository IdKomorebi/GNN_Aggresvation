"""
DNN_Aggresvation6 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（KNN+阈值边掩码）
4. 训练端到端推断驱动GNN/强target-conditioned GAT
...
def _save_attention_diagnostics(
    ...
    """保存GAT最后一层注意力诊断。"""
    if getattr(model, "architecture", "gcn") not in {"gat", "target_gat", "target_bilinear_gat"}:
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation6: 强target-conditioned GAT 推断驱动敏感度评估流水线"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation6: 强target-conditioned GAT 推断驱动敏感度评估")
    print("=" * 60)
    ...
    model = InferenceDrivenGNN(
        ...
        target_specific_heads=bool(model_cfg.get("target_specific_heads", False)),
    ).to(device)
    print(f"  模型架构: {architecture}")
    if architecture == "target_bilinear_gat":
        print(
            "  强条件参数: "
            f"attention_dim={int(model_cfg.get('attention_dim', model_cfg.get('hidden_dim', 32)))}, "
            f"target_specific_heads={bool(model_cfg.get('target_specific_heads', False))}"
        )
