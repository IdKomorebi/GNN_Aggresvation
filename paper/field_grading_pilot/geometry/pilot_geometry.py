...
def fit_geom(sets, v, d, seed, steps=2500):
    torch.manual_seed(seed)
    X = torch.tensor(X_of(sets), dtype=torch.float64)
    y = torch.tensor(np.clip(v, 0, 1), dtype=torch.float64)
    U = (0.3 * torch.randn(n, d, dtype=torch.float64)).requires_grad_()
    ar = torch.randn(d, dtype=torch.float64).requires_grad_(); s = torch.tensor(1.0, dtype=torch.float64, requires_grad=True)
    opt = torch.optim.Adam([U, ar, s], lr=0.03)
    def pred(Xb):
        a = ar / ar.norm() * torch.sigmoid(s * 3)
        P = torch.einsum("id,ie->ide", U, U)
        M = torch.einsum("bi,ide->bde", Xb, P) + torch.eye(d, dtype=torch.float64)
        sol = torch.linalg.solve(M, a.expand(len(Xb), d).unsqueeze(-1)).squeeze(-1)
        return a @ a - sol @ a
    for t in range(steps):
        opt.zero_grad()
        p = pred(X)
        loss = ((p - y) ** 2).mean() + 1e-5 * (U ** 2).sum()
        loss.backward(); opt.step()
    return (lambda sets2: pred(torch.tensor(X_of(sets2), dtype=torch.float64)).detach().numpy()), loss.item()
