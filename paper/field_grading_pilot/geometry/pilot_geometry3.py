...
for conf, items in sorted(by_conf.items()):
    ...
    wides = [x for x in items if x[1] == "wide_random" and len(x[0]) >= 3]
    wk = rng.random(len(wides)) < float(os.environ.get("WIDE_FRAC","0"))
    fit = [x for x in items if x[1] in ("single", "triple_rand")] + [p for p, k in zip(pairs, keep) if k] + [x for x, k in zip(wides, wk) if k]
    evsets = {"pair_held": [p for p, k in zip(pairs, keep) if not k],
              "top3": [x for x in items if x[1] == "triple_top"],
              "wide": [x for x, k in zip(wides, wk) if not k]}
    ...
    for d in DIMS:
        best = min((fit_geom(fs, fv, d, sd) for sd in range(int(os.environ.get("SEEDS","3")))), key=lambda t: t[1])
