...
def fig_estimator():
    ...
    ax.set_xlabel("backgrounds certified, $k$", fontsize=7); ax.set_ylabel("certified $L^{(2)}$ / exact $M^{(2)}$")
...
def fig_ablation():
    ...
    Sel = Sel[Sel.候选 == "C3"].rename(columns={"V误差_3": "V误差"}).assign(变体="C3")
    V = pd.concat([V, Sel[["数据", "目标", "变体", "V误差", "M2误差", "认证前1"]]], ignore_index=True)
    order = [("lin", "linear readout, no backbone"), ("polyS", "in-set quadratic, no backbone"), ("head3", "shared output head"),
             ...
             ("phionly1", "readout on $\\phi$ only"), ("nofix1", "no numerical safeguards"), ("D1", "target-only, one seed"),
             ("E", "target-only, three seeds"), ("C3", "ours")]
    ...
    for k, (col, lab, lim) in enumerate([("V误差", "MAE of $V$", (0, 0.23)), ("M2误差", "MAE of $M^{(2)}$", (0, 0.155)),
        ...
        cols = [S.BLUE if v == "C3" else (S.LIGHTBLUE if v in ("D1", "E") else ("#c8c8c8" if v in ("lin", "polyS", "head3") else "#8c8c8c")) for v, _ in order]
...
def fig_masking():
    ...
    lab = {"reconE": "reconstr. ×3", "reconE_cy": "reconstr. ×3 + clip", "recon+random_cy": "reconstr. ⊕ untrained ×1 + clip",
           "RRE_cy": "ours: reconstr. ⊕ untrained ×3 + clip"}
...
def fig_scope():
    ...
    order_b = ["本组主干", "全列主干", "留目标主干"]; lab_b = ["per-group", "all-column", "target held out"]; cb = ["#8c8c8c", S.BLUE, S.ORANGE]
    m = B.groupby("主干")[["死神经元比例", "有效维度", "V绝对误差", "边际Δ绝对误差", "M2误差", "认证前1", "认证前3"]].mean().reindex(order_b)
    fig, axs = plt.subplots(1, 3, figsize=(S.TEXTW, 2.1), gridspec_kw=dict(wspace=0.45)); x = np.arange(3); w = 0.26
    ax = axs[0]; ax.bar(x, m["死神经元比例"] * 100, 0.6, color=cb)
    for xi, v, dm in zip(x, m["死神经元比例"] * 100, m["有效维度"]):
        ax.text(xi, v + 0.8, f"eff. dim\n{dm:.2f}", ha="center", fontsize=5.6, color=S.INK2)
    ax.set_ylim(0, 36); ax.set_xticks(x); ax.set_xticklabels(["per-\ngroup", "all-\ncolumn", "target\nheld out"], fontsize=6.2); ax.set_ylabel("inactive units (%)", fontsize=7); S.ygrid(ax); S.panel(ax, "a", x=-0.3)
    ax = axs[1]
    for j in range(3):
        ax.bar(x + (j - 1) * w, m.iloc[j][["V绝对误差", "边际Δ绝对误差", "M2误差"]].values, w * 0.9, color=cb[j], label=lab_b[j])
    ax.set_xticks(x); ax.set_xticklabels(["$V$", "marginal $\\Delta$", "$M^{(2)}$"], fontsize=6.6); ax.set_ylabel("MAE", fontsize=7)
    ax.set_ylim(0, 0.085); ax.legend(fontsize=5.6, loc="upper left", ncol=1); S.ygrid(ax); S.panel(ax, "b", x=-0.3)
    ax = axs[2]
    for j in range(3):
        ax.bar(np.arange(2) + (j - 1) * w, m.iloc[j][["认证前1", "认证前3"]].values, w * 0.9, color=cb[j])
    ax.set_xticks(range(2)); ax.set_xticklabels(["top-1", "top-3"], fontsize=6.6); ax.set_ylim(0.6, 1.0); ax.set_ylabel("certified ratio", fontsize=7)
...
def fig_baselines():
    import importlib
    mt = importlib.import_module("make_tables"); T = mt._baseline_times()
    M = pd.read_csv(R("DNN_Aggresvation127/outputs/analysis/summary.csv"), index_col=0)
    NM = {"lin": "linear regression", "poly": "quadratic regression", "dropout": "mean substitution", "lazyvi": "LazyVI", "ws": "warm start",
          "surrogate": "masked surrogate", "tabpfn": "TabPFN v2", "ours": "ours", "retrain": "retraining"}
    STY = {"lin": ("#8c8c8c", "v"), "poly": ("#8c8c8c", "^"), "dropout": ("#8c8c8c", "X"), "ws": ("#8c8c8c", "D"),
           "surrogate": ("#8c8c8c", "P"), "tabpfn": (S.ORANGE, "o"), "ours": (S.BLUE, "o"), "retrain": (S.INK, "s"), "lazyvi": ("#8c8c8c", "*")}
    tm = {k: np.sqrt(T[k][0] * T[k][1]) for k in T}
    fig = plt.figure(figsize=(S.TEXTW, 2.9)); gs = fig.add_gridspec(1, 3, wspace=0.6, width_ratios=[1, 1, 0.95], bottom=0.3)
    axs = [fig.add_subplot(gs[i]) for i in range(3)]
    for ax, c, lab, lim in [(axs[0], "M2误差", "MAE of $M^{(2)}$", (-0.005, 0.26)), (axs[1], "认证前1", "certified ratio, top-1", (0.25, 1.04))]:
        for k in ["lin", "poly", "dropout", "ws", "surrogate", "tabpfn", "ours", "retrain"]:
            ...
            ax.errorbar(tm[k], y, xerr=[[tm[k] - T[k][0]], [T[k][1] - tm[k]]], fmt=mk, ms=4.5, color=cl, elinewidth=0.8, capsize=0,
                        label=NM[k], zorder=3 if k in ("ours", "tabpfn") else 2)
        ax.set_xscale("log"); ax.set_xlim(5e-5, 300); ax.set_ylim(*lim); ax.set_xlabel("time per field set (s)"); ax.set_ylabel(lab); S.ygrid(ax)
    S.panel(axs[0], "a", x=-0.3); S.panel(axs[1], "b", x=-0.3)
    h, l = axs[0].get_legend_handles_labels()
    fig.legend(h, l, loc="lower left", bbox_to_anchor=(0.02, 0.0), ncol=4, fontsize=5.8, handletextpad=0.3, columnspacing=1.0)
    ax = axs[2]; ks = sorted(["dropout", "lin", "poly", "surrogate", "ws", "lazyvi", "ours", "tabpfn"], key=lambda k: -M.loc[k, "样本V误差"])
    y = np.arange(len(ks)); ax.barh(y, [M.loc[k, "样本V误差"] for k in ks], 0.62, color=[STY[k][0] if k in ("ours", "tabpfn") else S.GRAYS[1] for k in ks])
    for yy, k in zip(y, ks):
        ax.text(M.loc[k, "样本V误差"] + 0.01, yy, f"{M.loc[k, '样本V误差']:.3f}", va="center", fontsize=5.6)
    ax.set_yticks(y); ax.set_yticklabels([NM[k] for k in ks], fontsize=6.0); ax.tick_params(axis="y", length=0); ax.set_xlim(0, 0.56)
    ax.set_xlabel("MAE of $V$ (200 sets)"); ax.grid(axis="x", color=S.GRID, lw=0.5); ax.set_axisbelow(True); S.panel(ax, "c", x=-0.62)
