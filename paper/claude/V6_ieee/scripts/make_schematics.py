...
def fig_framework():
    fig, ax = plt.subplots(figsize=(6.9, 3.0)); ax.set_xlim(0, 100); ax.set_ylim(0, 44); ax.axis("off")
    T, B = 7.0, 6.0
    ...
    box(ax, 55, 1.0, 23, 37.7, "Budgeted marginal risk",
        ...
        "$\\bullet$ single-field risk $M^{(0)}$\n$\\bullet$ amplification $\\Gamma^{(K)}=M^{(K)}-M^{(0)}$\n$\\bullet$ top backgrounds $T^{*}$\n"
    ...
    box(ax, 82, 27.8, 17.5, 10.9, "Grading", "grade by $M^{(K)}$, flag\namplified fields", "d", tsize=T, bsize=B)
    box(ax, 82, 14.4, 17.5, 10.9, "Withholding", "break the minimal\nunsafe sets; do not\nco-release with $T^{*}$", "d", tsize=T, bsize=B)
...
def fig_model():
    import numpy as np
    fig, ax = plt.subplots(figsize=(6.9, 2.7)); ax.set_xlim(0, 100); ax.set_ylim(0, 40); ax.axis("off")
    T, B = 7.0, 6.0
