...
WIDE = {"tab_risk.tex", "tab_design.tex", "tab_baselines.tex", "tab_backbone.tex", "tab_fields.tex"}   # IEEE 双栏：跨栏表


def w(name, s):
    """V6（IEEEtran 双栏）：宽表改为 table*（按 \\textwidth 缩放），其余为单栏表（按 \\columnwidth 缩放）。"""
    if name in WIDE:
        s = s.replace("\\begin{table}[!tbp]", "\\begin{table*}[!t]").replace("\\end{table}", "\\end{table*}")
    else:
        s = s.replace("\\begin{table}[!tbp]", "\\begin{table}[!t]").replace("\\resizebox{\\textwidth}", "\\resizebox{\\columnwidth}")
...
if __name__ == "__main__":
    for t in (sys.argv[1:] or ["risk", "decision", "ranking", "design", "baselines", "backbone"]):
        globals()[f"tab_{t}"]()
