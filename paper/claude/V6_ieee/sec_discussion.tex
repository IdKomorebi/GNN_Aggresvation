...
certification ensures that no flag rests on the estimate alone.
...
The amortized attacker is not required for the correctness of a flag: every field flagged by scan--certify is confirmed
...
earns its place when it has to reconstruct all columns: the representation then encodes the operating state, and combined
with the random bulk of an untrained network it improves every accuracy metric; its advantage grows with the size of the
queried sets. A maximum over many
estimated marginals is sensitive to rare readout failures rather than to average accuracy, so safeguards against
exploding predictions matter more for $\Mik$ than for $\Vy$. Among other ways of avoiding retraining, only a tabular foundation model used in context
matched it---slightly better on set values, worse on amplification, equal after certification---and it spends one to two
orders of magnitude more time on every set, because it re-processes the training rows of each set through a large network.
Such a model is a natural candidate for the attacker family itself.

\paragraph{Limitations}
\emph{(i)} Risk is defined relative to a fixed attacker family; a stronger attacker can only raise
$\Vy$ and hence the measured combination risk, but the numerical values are family-dependent. The in-context foundation
model of Section~\ref{sec:res-est} occasionally exceeded the retrained family, so the reported risks are lower bounds for
adversaries with access to such models.
...
may change the values of $\Mik$, although not the audit procedure; the estimator is also biased downward on the data set
with the fewest training rows. \emph{(iv)} $K=2$ nearly saturates the marginal risk on nine of ten targets but not threshold
...
members of small minimal unsafe sets. An amortized attacker---masked pre-training with column
reconstruction, the features of an untrained network and a closed-form readout per field set---scans a field set in
tens of milliseconds, and retraining a few scanned backgrounds per field certifies the result; it is more accurate than
estimators that re-use, linearize or fine-tune a full model and certifies as much of the risk as a tabular foundation
model at a small fraction of its cost. On four power data sets with rule-based field
...
whereas the scan--certify pipeline recovered 76--91\% of them with no false positives. The profile of
