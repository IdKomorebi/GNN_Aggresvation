...
def fig_model():
    ...
    box(ax, 0.5, 0.3, 31, 12.0, "Objective", "predict the hidden columns from the\nvisible ones; keep last hidden layer\nas frozen features $\\phi_r$ (seeds $r=1,2,3$)", "in", tsize=T, bsize=B)
    ...
    box(ax, 35, 0.3, 37, 12.0, "Estimate", "$\\hat y=\\frac{1}{3}\\sum_r Z_r\\beta_r$ ;  $\\hat V_y(S)=R^2_{\\mathrm{test}}(\\hat y)$ + closure\n"
        "safeguards: feature s.d. floor $10^{-3}$, clip to training range", "v", tsize=T, bsize=B)
