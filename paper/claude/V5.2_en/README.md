...
| 项目 | V5.1_en | **V5.2_en** |
| --- | --- | --- |
| 估计器对比的组织 | 一张大消融表，把本文各变体与无主干读出、共享输出头混在一起 | 拆成两块：**设计选择**（表 6 tab_design：特征映射 / 种子用法 / 数值防护 / 预训练掩码，每行带耗时）与**外部方法对比**（表 7 tab_baselines + 图 9 fig_baselines） |
| 外部 baseline | 只有热启动微调（一句话）与共享输出头 | 逐集合线性/二次回归、均值代替（Williamson）、LazyVI（Gao 等 ICML 2022）、热启动+早停（Sun & Raskutti）、掩码代理模型（Covert 等 JMLR 2021）、**TabPFN v2（Nature 2025）**；数据来自 127 号 |
| 结论 | — | 只有 TabPFN 与本文精度相当（V、M 略好；Γ 更差；认证持平），但每集合慢 13–110 倍；其余方法 M 误差为本文 3–16 倍 |
| 图 8（旧消融图） | 上半消融、下半主干复用 | 删去上半；下半独立为图 10 fig_scope |
| 表 tab_masking | 单独一张 | 并入设计表 |
| 讨论与局限 | — | 补"基础模型可作为攻击者族的一员"；局限 (i) 补"TabPFN 有时强于重训族，报告的风险对拥有此类模型的攻击者是下界" |
| 附录 | — | 新增各 baseline 的实现细节 |
| 参考文献 | — | 新增 hollmann2025tabpfn |
...
| 表 6 | tab_design | 设计选择 | 118、122、125 号；耗时 126 号 |
| 图 9 / 表 7 | fig_baselines / tab_baselines | 与外部方法对比 | 127 号 `outputs/analysis/summary.csv`、`time_*.csv`；126 号计时 |
| 图 10 | fig_scope | 主干复用范围 | 116 号 |
| 图 8 | fig_masking | 掩码预训练诊断 | 122 号 + 125 号 `variants_by_target.csv`、`error_decomposition.csv`、`rank_truncation.csv`、`large_sets.csv`、`decisions.csv` |
