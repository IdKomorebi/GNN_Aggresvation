...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.99\linewidth]{tail_blindness.png}}
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.99\linewidth]{framework.png}}
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.99\linewidth]{gap_and_ladder.png}}
...
\begin{figure}[htbp]
\centering
\centerline{\includegraphics[width=0.82\linewidth]{cost_fidelity.png}}
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.99\linewidth]{certify_highorder.png}}
...
该定标有直接的安全含义：\textbf{最危险的协同是低阶的}。同时它也修正了本文前置工作中两个先后出现的错误结论——基于 poly2 字典曾得到“三到四阶存在断崖式下跌”，基于 full 字典曾得到“高阶不衰减、风险被系统性低估”；认证表明两者都是\textbf{字典假象}。这段修正链本身是本文方法论主张的最好例证：没有重训认证做裁判，结构化估计的任何定量结论都可能是估计器的伪影。
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.92\linewidth]{decay_law.png}}
...
\begin{figure}[htbp]
\centering
\centerline{\includegraphics[width=0.8\linewidth]{fdr_filter.png}}
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.92\linewidth]{ensemble_filter.png}}
...
\begin{figure*}[htbp]
\centering
\centerline{\includegraphics[width=0.86\linewidth]{caiso_certify.png}}
...
\textbf{(b) $k=6$--$7$：卡在表征，不在阶数。} 真值端仍能量回（回收率 $0.25$--$0.28$，单调性违反率 $0\%$），说明尺子还没坏；而搜索端 $\varphi$ 在七阶跌到百分位 $0.4$--$6.8$（随机基线 50）。关键对照是：换成与目标无关的手工 full 字典，\textbf{同一批七阶信号仍被稳定挑到 400 个随机集合的第一名}（CAISO 六个元组 $6/6$ 命中，PJM $4/6$）。两臂唯一差别是特征基，因此六、七阶的搜索失败\textbf{不能归因于阶数}，而是 $\varphi$ 对未见过的交互型目标分布外——即第 \ref{sec:limit} 节局限 (1) 的量化版本。这也给出一条可证伪的改进路线：把合成高阶信号混入 L1 的元训练任务分布以改造 $\varphi$ 的训练分布，而非加大搜索预算或更换认证协议。
...
\textbf{(2) 注入实验区分“数据里没有”与“方法找不到”}。当搜索不到强高阶协同时，需要判断是数据本身稀疏还是估计器失明。做法是人工注入已知强度的合成协同信号，看方法能否找回。本文正是用它推翻了先前“高阶不可能被搜到”的不可能性论证。
...
(1) \textbf{$\varphi$ 不是通用基}。对分布外的合成目标信号，L0/L1 明显不如手工 full 字典（92 对 56 百分位），用合成目标增广训练也修不好（容量竞争）。第 \ref{sec:ceiling} 节把这条局限量化为可认证边界的直接成因：六、七阶上 $\varphi$ 的搜索百分位跌至 $0.4$--$6.8$ 而 full 仍为 100。两者互补而非替代，实践中可考虑显式混合。
...
(4) \textbf{$k\ge6$ 未解决，但成因已定位}：全枚举在六阶起不可行，而反层级协同的存在使 beam 搜索不可靠。第 \ref{sec:ceiling} 节进一步表明六、七阶的瓶颈在 $\varphi$ 的表征而非阶数本身，八阶起才是样本量决定的统计极限；相应地，$k=6$--$7$ 的可行方向是改造 $\varphi$ 的元训练任务分布，而 $k\ge8$ 在现有数据规模下无解。
...
最后，本文用注入定标给出了管线的可认证边界 $k^\ast=5$，并把边界之外的失败归因拆开：六、七阶的瓶颈在摊销表示对未见交互目标的分布外（换配基字典后同一信号仍可被稳定检出），指向"改造 $\varphi$ 的元训练任务分布"这一可证伪的改进路线；八阶起则是样本量决定的统计极限，与更大攻击者、更强模型族均无关。同一定标还暴露出认证协议本身的灵敏度随阶衰减，因而本文所报的衰减律倍数是下界——这既是一处必要的口径更正，也再次说明：在这一问题上，任何定量结论都必须连同量它的那把尺子一起报告。
