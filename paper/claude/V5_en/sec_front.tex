...
hidden field is distinguishable from a zero value. The objective combines predicting the sensitive targets from the visible fields with, optionally, reconstructing every
hidden column from the visible ones~\cite{yoon2020vime,he2022mae},
...
\end{equation}
Target-only pre-training ($\gamma=0$) is the estimator E used for the main results. We found that it collapses the
hidden layer onto the few directions that predict the targets on average over masks, and that on the small sets of an
audit it then adds nothing over an untrained network; reconstruction ($\gamma=1$) forces the representation to encode
the operating state that drives all fields and improves every accuracy metric (Section~\ref{sec:res-est}).
Reconstructing all columns the auditor holds also decouples the backbone from any particular target definition. The
last hidden layer (256 units) is kept as a frozen feature map $\phi_\theta(x\odot m, m)$; early stopping uses the target
loss on validation rows only.
...
needs a third safeguard, clipping predictions to the range of the target on the training rows. No test label enters
