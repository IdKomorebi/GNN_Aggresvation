...
def tab_masking():
    ...
    V = V.replace({"变体": SM}); Dd = Dd.replace({"估计器": SM})
    rows = [("random", "Untrained backbone", "random", 1), ("random_cy", "Untrained backbone, clip", "random", 1), ("uniformE", "Target-only, $\\times3$ (E, main results)", "uniform", 3),
            ("uniformE_cy", "Target-only, $\\times3$, clip", "uniform", 3), ("reconE", "Reconstruction, $\\times3$", "recon", 3),
            ("reconE_cy", "Reconstruction, $\\times3$, clip", "recon", 3), ("recon+random_cy", "Reconstruction $\\oplus$ untrained, clip", "--", 1)]
    diag = V.groupby("变体")["有效维度"].mean()
    body = []
    for v, lab, dk, nb in rows:
        g = V[V.变体 == v].mean(numeric_only=True)
        rc = Dd[(Dd.估计器 == v) & (Dd.规则 == "扫描—认证标记") & (Dd.δ == 0.0) & (Dd.τ == 0.5)].召回.mean() if v in set(Dd.估计器) else np.nan
        ed = "--" if dk == "--" else f"{diag[dk]:.2f}"
        rcs = f"{rc:.2f}" if rc == rc else "--"
        body.append(f"{lab} & {ed} & {g.V误差_3:.4f} & {g.V误差_4:.4f} & {g.M2误差:.4f} & {g.Γ2误差:.4f} & {g.认证前1:.3f} & {g.背景召回前3:.2f} & {rcs} \\\\")
    s = r"""\begin{table}[!tbp]
\centering\small
