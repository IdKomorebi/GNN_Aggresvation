...
def train_generic(model, Xtr, Ytr, Xte, Yte):
    """88号修正：早停改用【训练集内部划分的 validation】，测试集只在最后算 R²。

    原实现每个 epoch 用测试集 MSE 选最佳权重（GPT 指出，已接受），
    使"DNN 重训真值"对测试集有轻微过拟合。此处从 Xtr 内部切 15% 作 val；
    测试集在本函数内**完全不被读取**（保留形参仅为兼容调用处签名）。
    """
    opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=5e-4)
    n_all = len(Xtr)
    g = torch.Generator(); g.manual_seed(20260725)
    perm = torch.randperm(n_all, generator=g).to(DEV)
    n_val = max(int(n_all * 0.15), 1)
