...
ap = argparse.ArgumentParser(); ap.add_argument("--ds", required=True); ap.add_argument("--set", default="k3"); ap.add_argument("--scheme", required=True)
...
ck = (REPO / f"DNN_Aggresvation75/outputs/oracle_{a.scheme}_seed0.pt") if a.ds == "pjm" else (ROOT / f"outputs/oracle_{a.ds}_{a.scheme}_seed0.pt")
...
f = ROOT / f"outputs/est/{a.ds}_{a.set}_mask-{a.scheme}_s{a.shard}of{a.nshard}.npz"
...
log("ESTMASK", "DONE", f"{a.ds}_{a.set} mask-{a.scheme} sh{a.shard}/{a.nshard} n={len(idx)} {ms:.2f} ms/集合", ms_per_set=ms)
print(f"done mask {ms:.2f} ms/set", flush=True)
