"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation8）。

子项目8收缩子项目7中过于分散的逐边alpha参数：
- General-General: 使用一组全局alpha_G构造固定相关性边权，并做加权GCN聚合
- Confidential-Source: 每个Confidential目标学习一组alpha_c，再用目标条件化注意力读取source
- gated模式中，注意力分数由动态query-key匹配与目标专属关系先验通过门控自适应融合
"""
...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            ...
            self.query_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
            self.query_id_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
            self.key_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
            self.value_layers.append(nn.Linear(hidden_dim, hidden_dim, bias=False))
            # 等价于W_lambda[q || k || r] + b，但避免构造(B,C,N,2H+M)大张量。
            self.gate_q_layers.append(nn.Linear(hidden_dim, 1, bias=False))
            self.gate_k_layers.append(nn.Linear(hidden_dim, 1, bias=False))
            self.gate_r_layers.append(nn.Linear(self.n_metrics, 1, bias=True))
    ...
    def _confidential_messages(
        ...
        q = self.query_layers[layer_idx](h_conf) + self.query_id_layers[layer_idx](target_id)
        k = self.key_layers[layer_idx](h)
        v = self.value_layers[layer_idx](h)

        dynamic_score = torch.matmul(q, k.transpose(1, 2)) / math.sqrt(self.hidden_dim)
        prior_score = prior.unsqueeze(0).expand(B, -1, -1)
        valid = prior > 0

        if self.architecture == "hybrid_prior_only":
            score = prior_score
            gate = torch.zeros_like(score)
        elif self.architecture == "hybrid_no_gate":
            ...
            gate_logits = (
                ...
                + self.gate_r_layers[layer_idx](rel).squeeze(-1).unsqueeze(0)
            )
            gate = torch.sigmoid(gate_logits)
            score = gate * dynamic_score + (1.0 - gate) * prior_score
    ...
    def get_mean_gate(self, node_values: torch.Tensor) -> np.ndarray | None:
        if self.architecture != "hybrid_gated":
            return None
        was_training = self.training
        self.eval()
        with torch.no_grad():
