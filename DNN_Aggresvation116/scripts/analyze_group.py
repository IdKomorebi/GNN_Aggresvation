# -*- coding: utf-8 -*-
"""116 号分析（由 111 号 analyze_exp.py 改写：估计器换成新主口径 E〔本组主干〕，只在规模 ≤3 的集合上评估）。
原说明：111/112 号通用分析：正式真值（三攻击器 max + 单调闭包）→ 字段风险表、增益矩阵、单字段口径暴露、
...
HERE = os.path.dirname(os.path.abspath(__file__)); sys.path.insert(0, os.path.join(HERE, "..", "..", "DNN_Aggresvation111", "src"))
...
E = np.load(os.path.join(OUT, "est_group.npz")); s3 = E["sel"]; keys3 = [k for k, t in zip(keys, s3) if t]
Ve = pipe.closure_max(E["E"], keys3); Ve1 = pipe.closure_max(E["E1"], keys3); V3 = V[s3]
idx = {k: r for r, k in enumerate(keys)}
fields = list(range(p))
M, W, Dt, bks, bsz = pipe.m_table(V, keys, fields, KB)
Me_all, _, De, _, _ = pipe.m_table(Ve, keys3, fields, 2)
_, _, Dt3, _, bsz3 = pipe.m_table(V3, keys3, fields, 2)
...
for c, y in enumerate(targ):
    ...
    Mt, Mest, L1, L3 = pipe.certify(Dt3[:, :, c], De[:, :, c], bsz3, 2)
    rec.update(V_MAE=float(np.abs(Ve[:, c] - V3[:, c]).mean()), V_MAE_单种子=float(np.abs(Ve1[:, c] - V3[:, c]).mean()),
...
by_size = pd.DataFrame([dict(规模=s, 正式真值均值=float(V[sizes == s].mean()),
                             通用模型MAE=float(np.abs(Ve[sizes[s3] == s] - V3[sizes[s3] == s]).mean()) if s <= 3 else np.nan) for s in range(1, spec["kmax"] + 1)])
...
rs = np.random.RandomState(0); sel = rs.choice(len(V3), min(3000, len(V3)), replace=False)
for c, y in enumerate(targ):
    axs[0].scatter(V3[sel, c], Ve[sel, c], s=3, alpha=.35, color=cols[c % 3], label=short(y), rasterized=True)
...
fig.suptitle(f"图4  {TAG}：通用推断模型（新主口径 E）的保真度（真值为三攻击器专用重训；规模 ≤3）", x=.01, ha="left", fontsize=11.5)
...
md = [f"# {TAG}：自动生成的数值汇总（结论与解读见 CHANGELOG.md）\n",
      ...
      f"\n## 通用模型查询耗时\n\n新主口径 E（三种子预测平均）{float(E['sec_per_set'])*1000:.0f} ms/集合\n",
