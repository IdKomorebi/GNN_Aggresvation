...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        general_aggregation: str = "general_only",
    ) -> None:
        super().__init__()

        if general_aggregation not in {"general_only", "include_confidential"}:
            raise ValueError(
                f"Unknown general_aggregation: {general_aggregation}"
            )
        ...
        self.general_aggregation = str(general_aggregation)
    ...
    def compute_general_adjacency(self, row_normalize: bool = True) -> torch.Tensor:
        """General 节点入边的固定相关性边权。

        general_aggregation:
        - "general_only"（原始）：只保留 General-General 块，General 节点聚合时
          看不到 Confidential 邻居（前向阻断）。
        - "include_confidential"（DNN39）：General 行保留所有列（含 Confidential），
          让 General 节点也从 Confidential 的（推断中的）表示聚合——不阻断。
          Confidential 节点初始为可学 embedding（不含真值），故不泄露标签。
        """
        ...
        row_mask = torch.zeros_like(A)
        if self.general_aggregation == "include_confidential":
            row_mask[: self.n_general, :] = 1.0          # General 行保留所有列
        else:
            row_mask[: self.n_general, : self.n_general] = 1.0  # 只保留 GG 块
        A = A * row_mask
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        for layer_idx, (self_layer, gen_layer, conf_layer, bn) in enumerate(
            ...
            if self.general_aggregation == "include_confidential":
                # 不阻断：General 从所有节点（含 Confidential）聚合
                general_messages = torch.matmul(A_general[: self.n_general, :], h)
            else:
                general_messages = torch.matmul(
                    A_general[: self.n_general, : self.n_general], h[:, : self.n_general, :]
                )
