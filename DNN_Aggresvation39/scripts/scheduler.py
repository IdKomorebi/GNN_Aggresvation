#!/usr/bin/env python3
"""DNN39 调度器：4 GPU 并行跑 13 个 run（1 multi + 12 single）。

DNN39 = DNN32_test（window=1 基础版）+ general_aggregation=include_confidential
（不阻断：General 节点也从 Confidential 聚合）。
...
D39 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation39")
PY = "/data1/duhaocun/miniconda3/envs/Pytorch310_codex/bin/python"
GPUS = [0, 1, 2, 3]
LOG_DIR = D39 / "logs"
LOG_DIR.mkdir(exist_ok=True)

# 构建任务队列：single 先跑，multi 最后
configs = sorted((D39 / "configs").glob("*.yaml"))
configs = [c for c in configs if c.name != "multi.yaml"]
configs.append(D39 / "configs" / "multi.yaml")
...
print(f"DNN39 调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D39 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
