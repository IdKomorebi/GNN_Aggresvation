...
def main() -> None:
    ...
    if single_target:
        run_dir = output_root / "single" / single_target / timestamp
    else:
        run_dir = output_root / "multi" / timestamp
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation32_test: window_size=1 multi-target vs single-target")
    print("=" * 60)
    ...
    model = InferenceDrivenGNN(
        ...
        bipartite=use_bipartite,
        general_aggregation=str(model_cfg.get("general_aggregation", "general_only")),
    ).to(device)
    ...
    print(f"  bipartite: {use_bipartite}")
    print(f"  general_aggregation: {str(model_cfg.get('general_aggregation', 'general_only'))}")
