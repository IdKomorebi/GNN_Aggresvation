...
class InferenceDrivenGNN(nn.Module):
    ...
    def __init__(
        ...
        # 统一聚合消融（DNN51）：把整张图都换成同一种聚合方式，验证「分裂聚合」的必要性。
        #   none        : 默认分裂聚合（G-G GCN+先验 / G→C 门控 GAT+先验）。
        #   gcn_noprior : 全图 GCN，邻接=二值（仅图结构，无相关性先验）。
        #   gcn_prior   : 全图 GCN，邻接=相关性先验加权（共享 alpha_general）。
        #   gat_noprior : 全图 GAT，仅 q·k 注意力，无先验项。
        #   gat_prior   : 全图 GAT，门控混合 q·k 与 log(先验)（与当前 conf 聚合同机制）。
        # 图结构不变：所有节点都只从其 general 邻居聚合（与分裂设计一致）。
        self.unified_aggregation = str(unified_aggregation)
        if self.unified_aggregation != "none":
            valid = {"gcn_noprior", "gcn_prior", "gat_noprior",
                     "gat_prior", "gat_prior_static"}
    ...
    def _unified_messages(self, h: torch.Tensor, layer_idx: int) -> torch.Tensor:
        """统一聚合（DNN51 消融）：整张图用同一种聚合方式，源限定为 general 邻居。"""
        mode = self.unified_aggregation
        support = self.agg_support                                  # (N,N) 二值，仅 general 源

        if mode in ("gcn_noprior", "gcn_prior"):
            if mode == "gcn_noprior":
                A = support
            else:  # gcn_prior：相关性先验加权邻接（共享 alpha_general）
                alpha_g = self.get_alpha_general()
                A = torch.einsum("ijk,k->ij", self.metric_tensor, alpha_g) * support
            A = A / A.sum(dim=1, keepdim=True).clamp(min=1e-12)
            return torch.matmul(A, h)                              # (B,N,H)
        ...
        if mode in ("gat_prior", "gat_prior_static"):
            alpha_g = self.get_alpha_general()
            P = torch.einsum("ijk,k->ij", self.metric_tensor, alpha_g) * support
            prior_log = torch.log(P.clamp(min=self.prior_log_eps))
            prior_score = prior_log.unsqueeze(0).expand(score.shape[0], -1, -1)
            scale = torch.nn.functional.softplus(self.prior_scales[layer_idx])
            if mode == "gat_prior":
                # 全局动态：逐对 gate 在动态 q·k 与先验间混合
                gate_logits = (
                    self.gate_q_layers[layer_idx](q)
                    + self.gate_k_layers[layer_idx](k).transpose(1, 2)
                    + self.gate_p_layers[layer_idx](prior_score.unsqueeze(-1)).squeeze(-1)
                ...
                # 全局静态(no_gate)：动态 q·k + 单一全局 scale × log(先验)，无逐对 gate
