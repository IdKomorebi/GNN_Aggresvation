...
def train_model(
    ...
    gate_l1_lambda = float(cfg.get("training", {}).get("gate_l1_lambda", 0.01))
    ...
    # 低数据场景：固定 test，仅对训练集子采样（seed 固定，可复现）
    subsample_frac = float(cfg.get("training", {}).get("train_subsample_frac", 1.0))
    if subsample_frac < 1.0:
        n_tr = len(train_features)
        keep = max(1, int(round(n_tr * subsample_frac)))
        sel = np.random.default_rng(42).choice(n_tr, keep, replace=False)
        train_features = train_features[sel]
        train_targets = train_targets[sel]
        print(f"  [低数据] 训练子采样 frac={subsample_frac}: {n_tr} -> {keep}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, batch_y in train_loader:
            ...
            # 输入门控 L1 正则（gate 方法：g 与模型联合训练）
            if getattr(model, "use_input_gate", False):
                loss = loss + gate_l1_lambda * model.input_gate.abs().sum()
