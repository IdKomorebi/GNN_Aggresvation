"""
DNN_Aggresvation10 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（KNN+阈值边掩码）
4. 训练去relation-bias的hybrid bilinear gated prior GAT（任务：还原Confidential真实值）
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN10混合聚合的alpha、prior、gate诊断。"""
    ...
    print(f"  DNN9混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation10: no-relation-bias hybrid bilinear gated prior GAT 推断驱动敏感度评估流水线"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation10: no-relation-bias hybrid bilinear gated prior GAT 推断驱动敏感度评估")
    print("=" * 60)
