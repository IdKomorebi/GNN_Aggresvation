#!/usr/bin/env python3
"""DNN43 图结构扫描调度器：4 GPU 并行跑 configs/ 下的全部调参配置。
...
D43 = Path("/data1/duhaocun/projects/GNN_Aggresvation/DNN_Aggresvation43")
PY = "/data1/duhaocun/miniconda3/envs/Pytorch310_codex/bin/python"
GPUS = [0, 1, 2, 3]
LOG_DIR = D43 / "logs"
LOG_DIR.mkdir(exist_ok=True)

configs = sorted((D43 / "configs").glob("*.yaml"))

queue = deque(configs)
gpu_pool = list(GPUS)
running = {}  # gpu -> (proc, config_name)

print(f"DNN43 图结构扫描调度器：{len(configs)} 个任务，{len(GPUS)} 张 GPU")
...
def launch(cfg: Path, gpu: int):
    ...
    cmd = [PY, "-u", str(D43 / "scripts" / "run_pipeline.py"), "--config", str(cfg)]
