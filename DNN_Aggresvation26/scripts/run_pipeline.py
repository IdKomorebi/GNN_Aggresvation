"""
DNN_Aggresvation26 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（固定KNN或自适应阈值-KNN边掩码）
4. 训练short temporal window input的hybrid gated prior GAT
...
def _save_hybrid_diagnostics(
    ...
    """保存DNN26混合聚合的alpha、prior、gate诊断。"""
    ...
    print(f"  DNN26混合聚合诊断已保存至: {diag_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation26: staged phase length tuning pipeline"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation26: staged phase length tuning")
    print("=" * 60)
