"""
端到端推断驱动的混合GNN模型（DNN_Aggresvation16）。

子项目16沿用DNN13/DNN14的confidential target-conditioned attention主线，
只将General-General从固定加权GCN改为边先验GAT：
- General-General: 使用一组全局alpha_G构造相关性边先验，并做GAT聚合
...
class InferenceDrivenGNN(nn.Module):
    ...
    def _general_messages(
        ...
        """General-General聚合。DNN16默认使用带边先验的GAT。"""
        h_general = h[:, : self.n_general, :]
        if self.architecture != "hybrid_bilinear_gated_general_gat":
            A_gg = prior[: self.n_general, : self.n_general]
            row_sum = A_gg.sum(dim=1, keepdim=True).clamp(min=1e-12)
            A_gg = A_gg / row_sum
            self.last_general_attention = None
            return torch.matmul(A_gg, h_general)

        prior_gg = prior[: self.n_general, : self.n_general]
        valid = prior_gg > 0
        q = self.general_query_layers[layer_idx](h_general)
        k = self.general_key_layers[layer_idx](h_general)
        v = self.general_value_layers[layer_idx](h_general)

        dynamic_score = torch.matmul(q, k.transpose(1, 2)) / math.sqrt(self.attention_dim)
        prior_log = torch.log(prior_gg.clamp(min=self.prior_log_eps))
        scale = torch.nn.functional.softplus(self.general_prior_scales[layer_idx])
        score = dynamic_score + scale * prior_log.unsqueeze(0)
    ...
    def forward(self, node_values: torch.Tensor) -> torch.Tensor:
        ...
        general_prior = self.compute_general_adjacency(row_normalize=False)
    ...
    def get_mean_gate(self, node_values: torch.Tensor) -> np.ndarray | None:
        if self.architecture not in {"hybrid_bilinear_gated", "hybrid_bilinear_gated_general_gat"}:
            return None
        was_training = self.training
        self.eval()
        with torch.no_grad():
