"""
DNN_Aggresvation5 主流水线脚本。

完整流程：
1. 数据加载与预处理
2. 计算5种相关性指标张量
3. 构建图结构（KNN+阈值边掩码）
4. 训练端到端推断驱动GNN/target-conditioned GAT
...
def _save_attention_diagnostics(
    ...
    """保存GAT最后一层注意力诊断。"""
    if getattr(model, "architecture", "gcn") not in {"gat", "target_gat"}:
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DNN_Aggresvation5: target-conditioned GAT 推断驱动敏感度评估流水线"
    )
    parser.add_argument(
        "--config",
    ...
    print("\n" + "=" * 60)
    print("  DNN_Aggresvation5: target-conditioned GAT 推断驱动敏感度评估")
    print("=" * 60)
