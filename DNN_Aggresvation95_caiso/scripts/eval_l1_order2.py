...
TRUTH = ROOT / "outputs/synergy2_perconf.csv"
...
def main() -> None:
    ...
    df.to_csv(ROOT / f"outputs/eval_l1_order2{args.tag}.csv", index=False)
    pd.concat(curves).to_csv(ROOT / f"outputs/eval_l1_pairs{args.tag}.csv", index=False)
