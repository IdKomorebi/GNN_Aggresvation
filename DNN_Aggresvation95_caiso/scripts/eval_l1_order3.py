...
CKPT = ROOT / "outputs/oracle_uniform_seed0.pt"
POOL = ROOT / "outputs/h3_unbiased_pool.csv"
...
def main() -> None:
    ...
    ap.add_argument("--ckpt", default=str(CKPT), help="换 L1 checkpoint 做对比")
    ...
    df.assign(ckpt=ckpt.stem).to_csv(ROOT / f"outputs/eval_l1_order3{args.tag}.csv", index=False)
    pd.concat(dumps).to_csv(ROOT / f"outputs/eval_l1_triples{args.tag}.csv", index=False)
